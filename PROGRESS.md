...
- **오류 제보 2건(2026-09-08) — 위젯 입력칸 스페이스 삼킴 · 컴시간 "이번 주 변경" 안내** — 구현·게이트 완료, **미커밋**. 스페이스는 `SortableWidget` 손잡이 `setActivatorNodeRef`(REGRESSION #72). 컴시간은 일일자료(자료147)를 받아 배너·토스트로 알리기만(반영 안 함, [ADR-091](docs/03-decisions/ADR-091.md), #73). ★잔여: 실기기 4단계(기록 참조) · overrides 자동 등록은 별도 설계. [기록](docs/progress/2026-09.md).
