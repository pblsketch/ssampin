...
- ⚠️ **native-desktop-left-edge-resize-fix Phase 7-D 2차 fix 진행 (2026-05-23)** — 사용자 신고(준일님, 2026-05-23) "바탕화면 위젯 모드에서 위젯 왼쪽 테두리 드래그 시 위젯 한 번에 사라짐. 위젯 위치 초기화로 복귀". 진단: 단일 프레임 결정적 teleport → Phase 7-D 1차 fix(setBounds 채택, 2026-05-07)의 WS_CHILD 회귀 확정. drag 경로(native SetWindowPos, ASYNC)는 정상. **Fix**: `electron/platform/win32Desktop.ts`에 `moveAndResizeWidgetSync` 신규 export(SWP_ASYNCWINDOWPOS 제외 sync 변형). `electron/desktopWidgetManager.ts`의 resize MOUSEMOVE setBounds 호출을 이 함수로 교체. resize 산식은 `computeResizeBounds` 순수 함수로 추출. **테스트**: `electron/desktopWidgetManager.resize.test.ts` 5 케이스(left clamp on/off · right · top-left corner · SWP flag SSOT 메타테스트). **검증 게이트**: tsc 0 / lint 0 / 신규 테스트 5/5 / regression / 빌드 반영(dist-electron/main.js mtime + grep `moveAndResizeWidgetSync`). **사용자 확인 대기**: native-desktop 모드 left edge 5회 드래그(안→밖/밖→안/빠르게/끝까지) 무회귀. **연관 문서**: DECISIONS.md ADR-008. **Phase 7-D 이력**: 1차 fix(2026-05-07, setBounds 채택) → 2차 fix(2026-05-23, SetWindowPos sync 복귀).
