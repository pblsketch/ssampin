...
const QA_DOCUMENTS = [
  ...
  // ── v1.9.6 신규 ──────────────────────────────────────────────────────────
  {
    content: `Q: 칠판 도구는 어떻게 사용하나요?\nA: 쌤도구에서 '칠판'을 선택하면 전체화면 칠판이 열려요. 판서(펜), 텍스트 입력, 선택/이동, 지우개 기능을 지원해요. 펜 색상과 굵기를 바꿀 수 있고, 전체 지우기도 가능해요.`,
...
const FEATURE_DOCUMENTS = [
  ...
  {
    content: `🔧 쌤도구 (17가지+): 타이머(종료 예고 알림), 랜덤 뽑기, 신호등, 점수판, 룰렛, 주사위, 동전 던지기, QR코드 생성, 활동 기호, 투표, 설문/체크리스트, 워드클라우드, 자리 뽑기(번호 역순 옵션), 과제 수합, 칠판(판서/텍스트/선택/지우개), 가치수직선 토론, 신호등 토론. 효과음 지원(설정에서 켜기/끄기).`,
