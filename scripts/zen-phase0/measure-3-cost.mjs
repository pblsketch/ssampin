#!/usr/bin/env node
/**
 * Phase 0 비용 측정 — 질문 1건당 실제 토큰·비용
 *
 * 주의: 계획서 §7 의 "실측 ③"은 한국어 품질이다. 이 스크립트는 그것이 아니라
 * 계획서에 없던 비용 측정이다. 파일명의 3은 실행 순서일 뿐이다.
 *
...
// ★측정용 **대역** 프롬프트다. 운영 프롬프트를 여기 옮겨 적지 말 것 —
// 이 저장소는 공개이고, 운영 프롬프트 본문은 서버 시크릿에만 둔다(ADR-072 결정 1).
// 토큰 수 규모만 비슷하면 되므로 내용은 일반적인 문구로 유지한다.
const SYSTEM = [
  '당신은 교사의 업무를 돕는 비서입니다.',
