...
const presenceChecks = [
  ...
  // ────────────────────────────────────────────────────────────────────────
  // REGRESSION #69·#70 — 생기부 분량 조절 (ADR-088, 2026-09-07)
  //
  // 이 둘은 **동작 계약**이라 #65·#67 과 같은 이유로 grep 으로 지킬 수 없다.
  // `getSourceText` 라는 이름만 남고 안에서 저장된 글을 읽어도 grep 은 초록이 된다.
  // 진짜 방어선은 렌더 테스트(`recordDraftLengthAdjust.test.tsx`)이고, 여기서는
  // **그 테스트가 지워지거나 속이 비지 않았는지**만 확인한다.
  //
  ...
  // ────────────────────────────────────────────────────────────────────────
  {
    ...
    pattern:
      /저장이 거부된 초과 글을 그대로 조절 대상으로 넘긴다[\s\S]*?expect\(seen\[0\]\)\.toBe\(OVER_LIMIT\)/,
    name: 'REGRESSION #69: 분량 조절이 화면의 현재 글을 대상으로 삼는 테스트가 살아 있다 (저장이 거부된 초과 글을 실제로 그려서 확인)',
  },
  {
    file: 'src/adapters/components/RecordDraft/__tests__/recordDraftLengthAdjust.test.tsx',
    pattern:
  ...
  // ────────────────────────────────────────────────────────────────────────
  // REGRESSION #74·#75 (2026-09-09) — 쌤도구 팝업의 두 계약 (ADR-098)
  //
  //  #74 팝업 지원 도구는 전부 이관에 참여해야 한다. `POPUP_TOOL_IDS` 에 도구를 더해 놓고
  //      그 도구 파일에 `useToolPopupSlot` 을 안 붙이면, 창을 옮길 때 그 도구만 상태가
  //      통째로 사라진다. 화면은 멀쩡히 열려서 아무도 신고하지 않는다.
  //  #75 돌아온 상태를 "첫 렌더에만" 읽으면 안 된다. 본문이 이미 그 도구 화면이면 페이지가
  //      바뀌지 않아 부품이 다시 만들어지지 않고, 넘어온 스냅샷이 조용히 버려진다
  //      (실제로 겪은 결함 — 타이머가 05:00 으로 초기화됐다). 그 왕복을 그리는 테스트가
  //      살아 있는지 확인한다.
  // ────────────────────────────────────────────────────────────────────────
  {
    file: 'src/adapters/components/Tools/popup/__tests__/allToolsHandoff.test.tsx',
    pattern: /EXPECTED_SLOTS[\s\S]*?'tool-work-symbols': \['work-symbols'\]/,
    name: 'REGRESSION #74: 팝업 지원 9종이 각자 이관 칸을 등록하는지 확인하는 표가 살아 있다 (도구만 늘리고 상태 이관을 빠뜨리는 것 방지)',
  },
  {
    file: 'src/adapters/components/Tools/popup/__tests__/mainToolPopupHost.test.tsx',
    pattern:
      /본문이 그 도구 화면에 그대로 있어도, 팝업에서 돌아온 상태를 받는다[\s\S]*?expect\(seen\.at\(-1\)\)\.toEqual/,
    name: 'REGRESSION #75: 본문 페이지가 그대로일 때도 팝업 복귀 상태가 도착하는 테스트가 살아 있다 (복귀 시 진행 상태 소실 방지)',
  },

  // ────────────────────────────────────────────────────────────────────────
  // REGRESSION #82 (2026-09-09, ADR-098) — 타이머 종료 덮개는 반드시 실제로 칠해져야 한다.
  //
  //   `bg-sp-bg/90` 처럼 sp-* 토큰에 Tailwind 투명도 수식을 붙이면 규칙이 **아예 생성되지 않아**
  //   조용히 투명해진다(실측 `rgba(0,0,0,0)`). 종료 덮개가 그래서 뒤의 7xl 숫자를 못 가렸고,
  ...
  // REGRESSION #117 (2026-09-12, ADR-110 보강 5) — 마무리 지시는 **근거가 얇으면 붙지 않고**,
  //  붙을 때는 **되짚기 backstop 바로 앞**(요청서 꼬리)에 온다.
  //  ① 조건이 사라지면 근거가 부족한 학생 초안이 다시 사실 문장에서 끊긴다(459B → 227B 실측).
  //  ② 자리가 분량 블록으로 되돌아가면 마지막 문장이 결과로 닫히는 비율이 떨어진다(3회 중 1회).
  {
    file: 'src/domain/services/recordDraftPack.ts',
    pattern:
