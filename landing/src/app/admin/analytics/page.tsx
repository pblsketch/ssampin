...
async function fetchTotals(): Promise<{
  ...
  const totalsView = await fetchView<{ total_users: number; today_users: number }>(
...
async function fetchChatConversations(): Promise<
  Array<{
    session_id: string;
    role: string;
    content: string;
...
> {
  ...
  const res = await fetch(
    `${url}/rest/v1/ssampin_conversations?select=session_id,role,content,created_at,is_test,sources&order=created_at.desc&limit=1000`,
    {
      headers: {
        apikey: key,
        Authorization: `Bearer ${key}`,
...
export default async function AdminAnalyticsPage({
  ...
  const [
    ...
    fetchView<{
      ...
      unique_users: number;
      avg_per_user: number;
    }>('analytics_tool_ranking', { order: 'usage_count.desc' }),
    ...
    }>('analytics_session_duration', { order: 'date.desc', dateColumn: 'date', dateFrom, dateTo }),
    fetchRecentEvents(),
    fetchTotals(),
    fetchView<{
      tool_name: string;
      ...
      unique_users: number;
      avg_per_user: number;
    }>('analytics_tool_ranking_weekly', { order: 'usage_count.desc' }),
    fetchView<{ app_version: string; users: number; last_seen: string }>(
      'analytics_version_distribution',
      { order: 'users.desc' },
    ),
    fetchView<{
      cohort_date: string;
    ...
    }>('chatbot_daily_stats', { order: 'date.desc', dateColumn: 'date', dateFrom, dateTo }),
    fetchView<{ keyword: string; mention_count: number; unique_sessions: number }>(
      'chatbot_popular_topics',
      { order: 'mention_count.desc' },
    ),
    fetchView<{ depth_bucket: string; session_count: number; pct: number }>(
      'chatbot_depth_distribution',
    ),
    fetchView<{
      id: string;
      type: string;
    ...
    }>('chatbot_recent_escalations', { order: 'created_at_kst.desc' }),
    fetchView<{ confidence_level: string; response_count: number; pct: number }>(
      'chatbot_confidence_stats',
    ),
                ...
                {sessions.map((s) => (
                  <div key={s.date} className="bg-gray-800/50 rounded-lg p-3 space-y-1.5">
                    ...
                    <div className="grid grid-cols-3 gap-x-2 text-xs text-gray-400">
                      ...
                      <div>
                        중간값:{' '}
                        <span className="text-gray-300">{formatDuration(s.median_seconds)}</span>
                      </div>
                      <div>
                ...
                <>
                  ...
                  <div className="block md:hidden space-y-3">
                    {retention.map((r) => (
                      <div
                        key={r.cohort_date}
                        className="bg-gray-800/50 rounded-lg p-3 space-y-1.5"
                      >
                        <div className="flex items-center justify-between">
