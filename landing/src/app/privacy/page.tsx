        ...
        <>
          ...
          <ul>
            ...
            <li>
              ...
              기능입니다. <strong>초대받아 참여한 경우에만</strong> 동작합니다.
              <ul>
                <li>
                  <strong>클라우드에 저장되는 것</strong> — 부서 이름·소개, 참여하신 분의{' '}
                  <strong>Google 계정 이메일</strong>과 부서에서 쓰실 표시 이름, 초대 코드, 게시판
                  글·댓글, 토론방 안건과 찬반 의견, 회의록, 부서 일정·업무 분담 내용, 누가 언제
                  읽었는지에 대한 기록.
                </li>
                <li>
                  <strong>자료실·갤러리에 올린 파일은 쌤핀 서버에 저장되지 않습니다.</strong> 파일은
                  올리는 순간 <strong>부서 관리자 선생님의 Google Drive로 곧장</strong> 전송되며,
                  쌤핀 서버에는 파일 이름·크기·올린 사람 같은 표찰만 남습니다. 내려받으실 때도
                  파일은 Google에서 직접 오고 쌤핀 서버를 지나지 않습니다.
                </li>
                <li>
                  <strong>
                    ★ 파일에서 뽑아낸 글자가 한 벌 더 만들어집니다 — 반드시 알아두셔야 합니다.
                  </strong>{' '}
                  자료실은 파일 이름뿐 아니라 <strong>파일 안의 내용까지 검색</strong>할 수
                  있습니다. 이를 위해 한글·워드·엑셀·PDF 파일을 올릴 때{' '}
                  <strong>올리는 분의 컴퓨터 안에서</strong> 글자를 뽑아내고(앞부분 5만 자까지), 그
                  글자를 <strong>원본 파일과 같은 Google Drive에 별도 파일로</strong> 함께 올립니다.
                  그리고 검색을 위해 그 글자가{' '}
                  <strong>같은 부서 선생님들의 컴퓨터로 내려받아져 보관</strong>됩니다. 파일을
                  분석하는 일과 검색은 전부 각자의 컴퓨터에서 이루어지며,{' '}
                  <strong>이 글자는 쌤핀 서버에 저장되지 않습니다.</strong> 따라서 학생 이름 등이
                  담긴 문서를 올리시면 그 내용이 부서 선생님들의 컴퓨터에도 남게 되므로, 올리실
                  자료를 미리 살펴봐 주세요.
                </li>
                <li>
                  <strong>부서에서 나가거나 내보내진 경우</strong> — 그때까지 열람 권한을 받았던
                  Drive 파일의 권한이 회수되어 더 이상 열 수 없습니다. 다만 이미 각자 컴퓨터에
                  내려받아 둔 자료는 쌤핀이 회수할 수 없습니다.
                </li>
              </ul>
            </li>
            <li>
        ...
        <>
          ...
          <ul>
            ...
            <li>
              ...
              <strong>
                ③온라인 교무실 자료실·갤러리에 올린 파일과, 검색을 위해 그 파일에서 뽑아낸 글자의
                저장 (제11조)
              </strong>
              — 이 자료는 쌤핀 서버가 아니라 <strong>부서 관리자 선생님의 Google Drive</strong>에
              보관되므로, 그 시점에 국외로 이전됩니다.
            </li>
            <li>
