    ...
    <section className="bg-sp-bg py-20">
      <div className="mx-auto max-w-6xl px-6">
        <FadeIn>
          ...
          <h2 className="text-3xl font-bold text-sp-text md:text-4xl">교실에서 바로 느끼는 변화</h2>
        </FadeIn>

        <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
          {testimonials.map((t, i) => (
            <FadeIn key={t.name} delay={i * 0.08}>
              <div className="h-full rounded-2xl border border-sp-border bg-sp-card p-6 shadow-sm">
                ...
                <p className="text-sm leading-relaxed text-sp-muted/90">&ldquo;{t.quote}&rdquo;</p>
              </div>
            </FadeIn>
          ))}
        </div>
