...
/**
 * 모델에게 주는 시스템 지시 — **본문은 저장소에 두지 않는다**(ADR-072 결정 1).
 *
 * 이 저장소는 공개라, 프롬프트를 코드에 적어 두면 그대로 읽힌다. 본문은 서버 환경변수
 * `ASSIST_SYSTEM_PROMPT` 가 들고 있고 여기서는 자리표시자만 채운다.
 *
 * ★순수 함수로 둔다 — Deno 전역을 쓰지 않아야 vitest 쪽 테스트가 돈다(파일 머리 주석).
 * 그래서 env 읽기는 **호출부**(`ssampin-assist/index.ts`)가 하고, 이 함수는 템플릿을 인자로 받는다.
 *
 * ★오늘 날짜를 반드시 넣어 부른다. 날짜가 없던 시절 모델이 지난 기한(8/19)을 "아직 남아
 * 있다"고 답한 신고가 있었다(2026-08-23 오너) — 모델은 오늘이 며칠인지 모르기 때문이다.
 * 템플릿의 `{{today}}` 가 그 자리다.
 */
export class AssistPromptNotConfiguredError extends Error {
  constructor() {
    super('ASSIST_SYSTEM_PROMPT 가 설정되지 않았습니다');
    this.name = 'AssistPromptNotConfiguredError';
  }
}

export function buildAssistSystemPrompt(template: string, today: string): string {
  // 조용히 빈 프롬프트로 돌면 모델이 안전 지시 없이 답한다. 배포 실수를 여기서 끊는다.
  if (template.trim().length === 0) throw new AssistPromptNotConfiguredError();
  return template.replaceAll('{{today}}', today);
}
