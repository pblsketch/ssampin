...
### 📅 시간표
학급 시간표(5일 × N교시) + 교사 개인 시간표. NEIS 자동 연동(학교 검색 → 학년/반 선택), 과목별/학반별 색상 모드, 색상 커스터마이징(16가지), 보기 모드에서 과목 셀 클릭으로 색상 즉시 변경, 교사 시간표 수업 장소 표시, 교사 시간표 엑셀 업로드/다운로드(컴시간 양식 자동 감지).

### 🪑 자리배치
드래그 앤 드롭 자리 교환, 조건부 랜덤 배치(구역·분리·인접·고정), 짝꿍 모드(2인/3인), 실행 취소(20단계), 교사 시점 보기, Excel/한글 내보내기(좌우 반전 옵션).

### 📆 일정 관리
캘린더 + 리스트 뷰(일/주/월/학기/연), 일정 검색(키워드+연도 필터), 카테고리(수업·행사·시험·개인), NEIS 학사일정 자동 연동, 구글 캘린더 동기화, D-Day 카운트다운, 교시 범위 선택(시작~종료 교시 지정).

### 👨‍🏫 담임 업무 (5개 탭)
학생 기록(카테고리별 상담·행동·학업 기록, 후속 조치, 번호순/자리배치 뷰 토글), 설문/체크리스트(교사·학생 응답 모드, QR/URL, PIN 인증), 상담 예약(위자드 UI, 시간표 연동 자동 차단), 과제 수합(QR/URL 제출), 자리배치.

### 📚 수업 관리
수업반 생성·관리, 명렬표(NEIS 호환 양식, 엑셀 업로드 헤더 자동 감지, 미리보기 모달), 좌석배치, 진도·출결 관리(진도 기록 날짜별 그룹 헤더), 출결 태그 색상 구분, 설문/체크리스트, 과제 수합(Google Drive 연동), 결번 학생 표시.
...
### ⚙️ 설정
학교/학급 정보(NEIS 검색, 직접 설정으로 학원/유치원/대안학교 지원), 교시 시간, 좌석 배치(행×열, 짝꿍), NEIS 연동(시간표·급식·학사일정), 날씨 지역(전국 83개 시/군), PIN 잠금(기능별), 테마(Notion 라이트 기본 + 7 프리셋 + 커스텀), 위젯 스타일(카드 모서리·간격·배경색·글꼴 등 세밀 커스터마이징), 폰트(11가지 한글 폰트), Google Drive 동기화, 사이드바 순서/숨기기, 행사 알림 on/off, 자동 시작, 방해 금지, 앱 정보·릴리즈 노트.

### 📱 쌤핀 모바일
스마트폰에서 시간표·출결·메모·할일·일정 조회 가능한 PWA(m.ssampin.com). Google Drive로 PC 데이터와 자동 동기화. 모바일에서 출결 체크 가능. iOS Safari 완전 지원(v1.8.1에서 안정화).
...
**Q: Google 로그인할 때 "확인되지 않은 앱" 경고가 나와요.**
...
**Q: 개인 컴퓨터에서는 되는데 학교 컴퓨터에서만 안 돼요**
A: 학교 컴퓨터에는 보안 프로그램이 설치되어 있어서 구글 연결 과정의 일부가 차단될 수 있어요. 위 "학교 컴퓨터에서 구글 연결이 안 돼요" 답변을 참고해 주세요. 시간표, 메모, 좌석 배치 등 핵심 기능은 구글 연동 없이도 모두 사용 가능해요! 😊
