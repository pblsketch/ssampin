...
/**
 * 챗봇 시스템 프롬프트 — **본문은 저장소에 두지 않는다**(ADR-072 결정 1).
 *
 * 이 저장소는 공개라 코드에 적어 두면 그대로 읽힌다. 본문은 서버 시크릿
 * `CHAT_SYSTEM_PROMPT` 에 있고, 미설정 시 조용히 빈 프롬프트로 답하지 않고 끊는다 —
 * 프롬프트 없이 답하면 근거 없는 말과 전달 규칙 이탈이 그대로 사용자에게 간다.
 */
class ChatPromptNotConfiguredError extends Error {
  constructor() {
    super('CHAT_SYSTEM_PROMPT 가 설정되지 않았습니다');
    this.name = 'ChatPromptNotConfiguredError';
  }
}

function systemPromptBase(): string {
  const raw = Deno.env.get('CHAT_SYSTEM_PROMPT') ?? '';
  if (raw.trim().length === 0) throw new ChatPromptNotConfiguredError();
  return raw;
}
...
async function rerankDocuments(
  ...
  try {
    ...
    const text = await generateText({
      ...
      // ★순위를 매기는 일에 무작위성을 주면 안 된다. 1.0 이던 것을 0 으로 내린다.
      //
      // 2026-08-27 실측: 검색이 **1위로 찾아낸 문서**를 이 단계가 버리고 있었다.
      // 같은 질문·같은 후보 10건으로 5회 돌린 결과 —
      //   temperature 1.0 → 1회는 정답을 통째로 탈락, 남은 회차도 순서가 제각각
      //                     ([0] / [0,6,7] / [6,1,3,0,7])
      //   temperature 0   → 5회 모두 정답이 1순위 ([0,7,8] 로 사실상 고정)
      // 여기서 밀린 문서는 답변 컨텍스트에 아예 못 들어가므로 챗봇이 "그 기능을 모른다"고
      // 답한다. v2.4.5 신규 Q&A 7건이 검색은 되는데 답변에 안 쓰이던 원인이 이것이었다.
      //
      // ★HyDE(가상 답변)는 범인이 아니었다 — 같은 실측에서 원본 100% 검색과
      // 원본40%+HyDE60% 검색 모두 정답이 1위였다. 그래서 손대지 않는다.
      temperature: 0,
      maxOutputTokens: 50,
      reasoning: 'minimal',
      timeoutMs: 15_000,
...
async function generateAnswer(
  ...
  const systemPrompt = systemPromptBase() + contextNote + platformNote;
