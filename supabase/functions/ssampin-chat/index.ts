...
2. **설문/체크리스트**: 교사 모드(화면에서 체크) 또는 학생 공유 모드(QR/URL로 학생이 응답)
3. **상담 예약**: 시간대 설정 → 학생/학부모가 URL로 예약
4. **자리배치**: 담임 교실 좌석배치 (위 자리배치와 동일)

### 📚 수업 관리
- 수업반(수업 클래스) 생성 및 관리
- 수업반별 명렬표, 좌석배치, 진도 관리, 출결 관리
- 과제 수합: Google Drive 연동, QR 코드로 학생 제출
...
### 🔧 쌤도구 (14가지+)
타이머, 랜덤 뽑기, 신호등, 점수판, 룰렛, 주사위, 동전 던지기, QR코드 생성, 활동 기호, 투표, 설문/체크리스트, 워드클라우드, 자리 뽑기, 과제 수합
...
- 오프라인 완전 동작 (날씨/급식/과제수합 제외)
- Windows 전용 데스크톱 앱 (Electron)
