...
### 📅 시간표
학급 시간표(5일 × N교시) + 교사 개인 시간표. NEIS 자동 연동(학교 검색 → 학년/반 선택), 과목별/학반별 색상 모드, 색상 커스터마이징(16가지), 교사 시간표 수업 장소 표시, 교사 시간표 엑셀 업로드/다운로드(컴시간 양식 자동 감지).
...
### 📚 수업 관리
수업반 생성·관리, 명렬표(NEIS 호환 양식, 엑셀 업로드 헤더 자동 감지, 미리보기 모달), 좌석배치, 진도·출결 관리, 출결 태그 색상 구분, 설문/체크리스트, 과제 수합(Google Drive 연동), 결번 학생 표시.
