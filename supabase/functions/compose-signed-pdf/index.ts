...
async function loadKoreanFont(): Promise<Uint8Array> {
  ...
  if (!res.ok) {
    throw new Error(`Noto Sans KR fetch 실패: HTTP ${res.status}`);
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.requestId || typeof body.requestId !== 'string') {
      return errorResponse('requestId 가 필요합니다.', 400);
    }
    ...
    if (request.admin_key !== body.adminKey) {
    ...
    if (!pdfTemplate || typeof pdfTemplate.storagePath !== 'string' || !pdfTemplate.pageCount) {
    ...
    for (const region of regions) {
      ...
      if (img) {
        try {
          const embedded =
            img.mime === 'image/webp'
              ? await doc.embedPng(img.bytes).catch(async () => await doc.embedJpg(img.bytes))
    ...
    const { data: latest, error: latestErr } = await supabase
      ...
      .limit(1);
    if (latestErr) return internalErrorResponse('compose-signed-pdf latest version', latestErr);
    const nextVersion = (latest?.[0]?.version ?? 0) + 1;

    // 7) 업로드 (idempotent upsert)
    const sanitized = sanitizeFileNameSegment(String(request.title ?? 'signature'));
    const fileName = `${sanitized}_쌤핀_행정용_v${nextVersion}.pdf`;
    const storagePath = `${body.requestId}/v${nextVersion}.pdf`;
    ...
    // 8) signature_compositions INSERT
    ...
    if (insErr) return internalErrorResponse('compose-signed-pdf composition insert', insErr);
