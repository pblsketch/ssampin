 ...
 * - 3: **글의 구성·순서·시작 방식을 요청서로 넘긴다**(ADR-099, 2026-09-09). 규정은 사실성·시점·문체·
 *      기재 금지·점검만 정하고, 무엇을 어떤 차례로 쓸지는 앱이 보내는 「작성 구성」이 정한다.
 *      ★앱은 이 값이 3 미만이면 새 구성을 **아예 쓰지 않고** 기존형으로 되돌린다
 *      (`RECORD_STYLE_MIN_PROMPT_VERSION`). 그래서 이 상수를 올리는 배포와 시크릿 교체는
 *      **함께** 해야 하고, 되돌릴 때도 함께 되돌린다.
 */
