 ...
 *
 * ⚠️ 모든 식별은 `subjectKey` 로 한다(`photoSubjectKey` 로 만든다).
 * **학번만으로 찾는 메서드를 여기에 추가하지 말 것** — 학번은 전학·번호 재부여·학년도 전환에서
 * 다른 학생을 가리키게 되고, 담임 명단에서는 아예 식별자가 아니다.
 */
export interface IStudentPhotoRepository {
  /** 전체 사진 메타 목록 */
  list(): Promise<readonly StudentPhoto[]>;

  /** 메타 + 사진 바이너리를 함께 저장 (같은 subjectKey 가 있으면 교체) */
  ...
  readPhoto(subjectKey: string): Promise<Uint8Array | null>;
