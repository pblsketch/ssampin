...
export function buildStyleInstruction(resolved: ResolvedComposition): string {
  ...
  // 오너 검토(2026-09-09, 예시 7편): 어떤 장면에서 있었던 일인지가 빠져 있었다. 단원·과제 이름이
  // 근거에 있어도 모델이 생략한다. 짝 제약: 근거에 없으면 지어내지 않고, 소개를 학생의 일로 바꾸지 않는다.
  // 오너 검토(2026-09-12): 여기서 과목·수업 이름은 뺀다 — 생활기록부 항목에 이미 적혀 있어 군더더기다.
  lines.push(
    '어떤 단원·과제·프로젝트에서 있었던 일인지를 첫 수행 항목의 첫 구절에서 밝힙니다' +
      "(예: '주장하는 글 쓰기 단원에서 급식 잔반 문제를 주제로 쓴 초안은'). " +
      '근거나 주제 정보에 그 이름이 있을 때만 쓰고, 없으면 지어내지 않고 활동의 종류만 씁니다. ' +
      '소개는 한 구절로 그치고, 수업의 목표나 과제 안내를 학생이 한 일처럼 쓰지 않습니다. ' +
      "과목·수업 이름은 쓰지 않습니다(✗ '독서와 작문 시간 팩트체크 활동에서' / ○ '팩트체크 활동에서').",
  );
  // 오너 검토(2026-09-09): "빈자리의 차이를 읽는 자리에 섬" 같은 문장은 뜻이 잡히지 않는다.
  lines.push(
    '뜻이 바로 잡히게 씁니다. 자리·몫·층위·빈자리처럼 뭉뚱그리는 낱말이나 ' +
      "'말해 주다·남겨 두다'처럼 뜻이 넓은 서술어로 사실을 대신하지 않습니다. " +
      '무엇을 설명하고 무엇을 생략하는지처럼 구체적으로 씁니다' +
      "(✗ '각 매체가 남기는 빈자리의 차이를 읽는 자리에 섬' / " +
      "○ '원작은 서술자가 설명해 주므로 독자가 상상할 부분이 적고, 영상은 설명이 없어 독자가 채울 부분이 많다고 결론지음'). " +
      '읽는 사람이 한 번에 이해하는 것이 먼저입니다.',
  );
