...
export function buildStyleInstruction(resolved: ResolvedComposition): string {
  ...
  // 오너 검토(2026-09-09, 예시 7편): 어떤 수업에서 있었던 일인지가 빠져 있었다. 수업 이름이 근거에
  // 있어도 모델이 생략한다. 짝 제약: 근거에 없으면 지어내지 않고, 수업 소개를 학생의 일로 바꾸지 않는다.
  lines.push(
    '어떤 수업·단원·과제·프로젝트에서 있었던 일인지를 첫 수행 항목의 첫 구절에서 밝힙니다' +
      "(예: '주장하는 글 쓰기 단원에서 급식 잔반 문제를 주제로 쓴 초안은'). " +
      '근거나 주제·과목 정보에 그 이름이 있을 때만 쓰고, 없으면 지어내지 않고 활동의 종류만 씁니다. ' +
      '수업 소개는 한 구절로 그치고, 수업의 목표나 과제 안내를 학생이 한 일처럼 쓰지 않습니다.',
  );
