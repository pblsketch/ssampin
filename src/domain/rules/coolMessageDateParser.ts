...
const DATE_ABS =
  /(?:(?<y>20\d{2})\s*[년./-]\s*)?(?<m>1[0-2]|0?[1-9])\s*(?<sep>[월./-])\s*(?<d>3[01]|[12]\d|0?[1-9])\s*(?<il>일)?\s*(?:\(\s*(?<wd>[월화수목금토일])\s*\))?/g;
 ...
 */
const TIME =
  /(?:(?<ampm>오전|오후|낮|저녁|밤|아침)\s*)?(?:(?<h1>2[0-3]|1\d|0?\d):(?<min1>[0-5]\d)(?!\d)|(?<h2>2[0-3]|1\d|0?\d)\s*시\s*(?:(?<min2>[0-5]?\d)\s*분|(?<half>반))?)/g;
