 ...
 *    친절했다. 위치를 똑같이 적는다.
 *
 * ★ 문장에 "일정"이라는 말을 쓰지 않는다(실기기 확인 2026-09-09에서 걸렸다).
 *   같은 문구를 상담과 설문이 함께 쓰는데, "이 일정을 만든 계정"이라고 적어 두면
 *   설문 화면에서 "이 일정"이라고 뜬다. 무엇을 못 열었는지는 배너 제목이 이미
 *   말한다("명단을 불러오지 못했습니다" / "응답을 불러오지 못했습니다").
 */
export const CONSULTATION_DENIAL_TEXT: Record<ConsultationDenialReason, string> = {
  ...
  different_account:
    '만든 사람의 구글 계정으로만 열 수 있습니다. 설정 > 구글 연결에서 그 계정으로 바꿔 주세요.',
  legacy_closed:
    '예전 방식으로 열 수 있는 기간이 끝났습니다. 이 기기에 받아 둔 사본이 있으면 아래에 표시됩니다.',
  key_mismatch: '관리 키가 일치하지 않습니다. 이 일정·설문을 만든 기기에서 다시 시도해 주세요.',
  owner_bound:
    '만든 선생님의 구글 계정으로만 열립니다. 이 화면이 계속 뜬다면 이 기기의 쌤핀을 최신 버전으로 업데이트한 뒤 다시 열어 주세요.',
};
