 ...
 *
 * ★**생기부 초안 저장은 여전히 만들지 않는다**(ADR-072) — 등록 누락이 아니라 존재하지
 * 않는다. 생기부는 서술 자체를 모델에 보내야 하는 일이라 성격이 다르다.
 *
 * 출결·관찰·채점은 열되(2026-08-25), 학생을 가리키는 말이 **번호와 별칭뿐**이라는 조건을
 * 함께 건다. 명단 조회 도구(list_students)는 등록하지 않는다 — 모델은 누가 있는지
 * 모르는 채로 선생님이 말한 한 명만 가리킬 수 있다.
 */
const WRITE_TOOLS: readonly AssistToolDef[] = [
  ...
  // -- 출결 (1) --
  // 학생에게 닿는 첫 쓰기다. 대상은 **번호 또는 선생님이 말한 이름**으로만 가리킨다 --
  // 모델에게 명단을 준 적이 없으므로 스스로 누구를 고를 수는 없다.
  writeTool(
    'set_attendance',
    '학생의 출결을 결석·지각·조퇴·결과로 표시한다(출석부에 적는다). 어떤 학생인지 student 에 번호나 이름으로 적는다. 확인 버튼을 누른 뒤에 저장된다.',
    {
      student: S('대상 학생. 번호("15번") 또는 선생님이 말한 이름'),
      status: S('처리: 결석 | 지각 | 조퇴 | 결과 | 출석'),
      date: S('YYYY-MM-DD 날짜(생략 시 오늘)'),
      period: N('교시. 0 은 조회, 9 는 종례. 생략하면 담임 학급의 하루 전체'),
      className: S('수업반 이름(생략 시 담임 학급)'),
      reason: S('사유: 질병 | 인정 | 미인정 | 기타'),
      memo: S('메모'),
    },
    ['student', 'status'],
  ),
  ...
  // -- 관찰 (1) --
  // 내용은 **선생님이 말한 문장**이다. 모델에게 관찰문을 짓게 하지 않는다 --
  // 학생에 대한 서술을 AI 가 대신 쓰는 것은 선을 그어 둔 자리다(ADR-072).
  writeTool(
    'add_observation',
    '수업 중 본 학생의 모습을 관찰 기록으로 남긴다. 내용은 선생님이 말한 문장을 그대로 옮긴다 — 지어내지 않는다. 확인 버튼을 누른 뒤에 저장된다.',
    {
      student: S('대상 학생. 번호("7번") 또는 선생님이 말한 이름'),
      content: S('관찰 내용. 선생님이 말한 문장을 그대로 옮긴다'),
      className: S('수업반 이름(수업반이 하나뿐이면 생략 가능)'),
      date: S('YYYY-MM-DD 날짜(생략 시 오늘)'),
      category: S('분류: 수업 관찰 | 상담·관계 | 생활·학습 | 기타'),
      tag: S('태그: 교과역량 | 학습태도 | 진로흥미 | 특이사항'),
    },
    ['student', 'content'],
  ),

  // -- 루브릭 채점 (1) --
  // 스토어 함수가 토글이라, 실행 직전에 지금 상태를 다시 본다(executeAssistWrite).
  // 이미 그 수준이면 부르지 않는다 -- 그대로 누르면 체크가 **풀린다**.
  writeTool(
    'set_rubric_mark',
    '평가 기준표(루브릭)에서 학생에게 수준을 체크한다. criterion 을 적으면 그 평가 요소 하나만, **비워 두면 기준표의 평가 요소 전부**에 체크한다("만점으로 해줘"는 criterion 을 비우고 level 에 "만점"을 넣는다). 확인 버튼을 누른 뒤에 저장된다.',
    {
      student: S('대상 학생. 번호("7번") 또는 선생님이 말한 이름'),
      rubric: S('평가 기준표 이름'),
      criterion: S('평가 요소 이름(예: 주장의 명확성). 요소 전부에 매길 때는 비워 둔다'),
      level: S('수준 이름(예: 잘함). 제일 높은 수준을 뜻할 때는 "만점"'),
    },
    // ★criterion 은 더는 필수가 아니다 — "만점으로 표시해줘"가 실사용에서 그냥 막혔다
    //   (2026-08-25). 만점은 애초에 요소를 하나 고르는 말이 아니다.
    ['student', 'rubric', 'level'],
  ),
];

/** 읽기 19종 + 쓰기 25종. 여기 있는 것만 모델에게 보인다. */
