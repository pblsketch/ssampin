...
import { RecordDraftConfirmedError, useRecordDraftsStore } from '../useRecordDraftsStore';
...
describe('★한도를 넘어도 저장한다 — 색으로만 알린다 (오너 결정 2026-09-11, ADR-105)', () => {
  const long = (bytes: number): string => '가'.repeat(Math.ceil(bytes / 3));
  afterEach(() => {
    useSettingsStore.setState((s) => ({ settings: { ...s.settings, recordLimitBytes: {} } }));
  });

  it('고등 과목세특 1,500B 를 넘어도 저장되고 바이트 수가 남는다', async () => {
    ...
    expect(rec?.byteLength).toBe(1503);
  });

  it('직접 정한 한도(750B)를 넘어도 저장된다', async () => {
    useSettingsStore.setState((s) => ({
      settings: { ...s.settings, schoolLevel: 'high', recordLimitBytes: { 'c1:subject': 750 } },
      loaded: true,
    }));
    const id = await useRecordDraftsStore
      .getState()
      .upsert({ area: 'subject', studentRef: 'L1', classId: 'c1', content: long(900) });
    expect(useRecordDraftsStore.getState().exists(id)).toBe(true);
  });

  it('level 을 넘기지 않는 브릿지 쓰기도 한도로 막지 않는다', async () => {
    useSettingsStore.setState((s) => ({
