...
import { upsertOverride, dedupeOverridesKeepLatest } from '@domain/rules/timetableRules';
...
class FakeStorage implements IStoragePort {
  ...
  async readBinary(_relPath: string): Promise<Uint8Array | null> {
    return null;
  }

  async writeBinary(_relPath: string, _bytes: Uint8Array): Promise<void> {
    /* no-op */
  }

  async removeBinary(_relPath: string): Promise<void> {
    /* no-op */
  }

  async listBinary(_dirRelPath: string): Promise<readonly string[]> {
    return [];
  }
}
  ...
  describe('S1: override write + re-read round-trip (file-byte equivalent)', () => {
    it('writes overrides to timetable-overrides key and reads back identically', async () => {
      const data: TimetableOverridesData = {
        overrides: [
          mk({ id: 'a', date: '2026-04-22', period: 3, subject: '자습', reason: '출장' }),
        ],
      };
