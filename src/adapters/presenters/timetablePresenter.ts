...
export function isLightTheme(): boolean {
  const bg = getComputedStyle(document.documentElement).getPropertyValue('--sp-bg').trim();
  if (!bg) return false;
 ...
 */
export function getSubjectStyle(subject: string, userColors?: SubjectColorMap): SubjectStyle {
...
export function getSubjectDotColor(subject: string, userColors?: SubjectColorMap): string {
  return resolvePreset(subject, userColors).tw.bgSolid;
}

/** 텍스트 색상 (대시보드 시간표용) */
export function getSubjectTextColor(subject: string, userColors?: SubjectColorMap): string {
  const p = resolvePreset(subject, userColors);
  return isLightTheme() ? p.tw.textOnLight : p.tw.text;
}

/** 위젯용 bg + textLight 합친 클래스 문자열 */
export function getSubjectWidgetStyle(subject: string, userColors?: SubjectColorMap): string {
  const p = resolvePreset(subject, userColors);
 ...
 *
 * 우선순위:
 * 1. lunchAfterPeriod (1-based, 사용자가 명시한 위치)
 * 2. lunchStart/lunchEnd 시간대와 교시 간 간격이 겹치는 지점
 * 3. 연속 교시 간 간격이 30분 이상인 첫 지점 (하위 호환)
 *
 * @returns 0-based 인덱스. 해당 인덱스의 교시 *앞에* 점심 행이 삽입된다.
 *          예: lunchAfterPeriod=4 → 반환 4 (인덱스 4 위치, 즉 5교시 앞)
 *          못 찾으면 -1.
 */
export function getLunchBreakIndex(
  ...
  // ① lunchAfterPeriod 우선 — 1-based 값이 곧 점심 행 삽입 0-based 인덱스
  // (lunchAfterPeriod=N이면 N교시 후 = 인덱스 N 위치에 점심 행 삽입)
  if (
    lunchAfterPeriod !== undefined &&
    lunchAfterPeriod >= 1 &&
    lunchAfterPeriod < periodTimes.length
  ) {
    return lunchAfterPeriod;
  }

  // ② lunchStart/lunchEnd 시간 겹침
