...
export function ToolDice({ onBack, isFullscreen }: ToolDiceProps) {
  ...
  const roll = useCallback(() => {
    ...
    // 1개 주사위일 때만 직전 결과 회피 (anti-repeat 항상 ON)
    // 2~3개일 땐 조합 회피 비용 대비 효과 낮아 secureRandom 균등
    ...
    <ToolLayout title="주사위" emoji="🎲" onBack={onBack} isFullscreen={isFullscreen}>
      <div className="w-full h-full flex flex-col items-center gap-6 overflow-auto py-4">
        ...
        <button
          onClick={roll}
          disabled={isRolling}
          className="px-10 py-4 rounded-2xl bg-sp-accent text-sp-accent-fg text-xl font-bold shadow-sp-md hover:bg-sp-accent/90 transition-all disabled:opacity-60 disabled:cursor-not-allowed active:scale-95 transform select-none"
        >
          {isRolling ? '던지는 중...' : '🎲 던지기!'}
        </button>
