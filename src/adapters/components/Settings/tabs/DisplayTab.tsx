...
export function DisplayTab({ draft, patch }: Props) {
  const patchDashboardTheme = useCallback(
    (t: DashboardThemeSettings) => {
      patch({ dashboardTheme: t });
    },
    [patch],
  );

  const patchWidgetStyle = useCallback(
    (s: WidgetStyleSettings) => {
      patch({ widgetStyle: s });
    },
    [patch],
  );
  ...
  const themeColors: ThemeColors = useMemo(() => {
    ...
    if (pid === 'custom' && draft.dashboardTheme?.customColors)
      return draft.dashboardTheme.customColors;
    return PRESET_THEMES[0]!.colors;
  }, [draft.dashboardTheme]);

  const updateStyle = useCallback(
    (p: Partial<WidgetStyleSettings>) => {
  ...
  return (
    <SettingsSection icon="palette" iconColor="bg-yellow-500/10 text-yellow-500" title="디스플레이">
      <div className="grid grid-cols-1 gap-8">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <GlassControls
              widget={draft.widget}
              onPatch={(p) => patch({ widget: { ...draft.widget, ...p } })}
            />
          </div>
        </div>
          ...
          <div className="flex bg-sp-surface/80 p-1 rounded-lg border border-sp-border">
            {(
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1 py-2.5 rounded-md text-sm font-medium transition-all ${
