...
function SeatCard({
  ...
  const pairRadiusClass = isLeftOfPair
    ...
    : isRightOfPair
      ...
      : 'rounded-lg';
  const pairBorderClass = isLeftOfPair ? 'border-r-0' : isRightOfPair ? 'border-l-0' : '';

  /* 빈 자리 */
  if (isEmpty && !isEditing) {
    return (
      <div
        className={`group relative border-2 border-dashed p-3 flex flex-col items-center justify-center gap-1 min-h-[90px] transition-all border-sp-border/50 bg-sp-card/50 ${pairRadiusClass} ${pairBorderClass}`}
      >
  ...
  if (isEditing && isEmpty) {
    return (
      <div
        className={`relative border-2 border-dashed p-3 flex flex-col items-center justify-center gap-1 min-h-[90px] transition-all ${pairRadiusClass} ${pairBorderClass} ${
          isDragOver ? 'border-sp-accent bg-sp-accent/10' : 'border-sp-border/50 bg-sp-card/50'
        }`}
  ...
  if (isEditing) {
    return (
      <div
        className={`relative border p-3 flex flex-col gap-2 min-h-[90px] transition-all cursor-grab active:cursor-grabbing ${pairRadiusClass} ${pairBorderClass} ${
          isDragSource
  ...
  return (
    <div
      className={`group relative border p-3 flex flex-col gap-2 shadow-sm transition-all min-h-[90px] border-sp-border bg-sp-card hover:border-sp-accent/50 hover:ring-1 hover:ring-sp-accent/50 ${pairRadiusClass} ${pairBorderClass}`}
    >
      {/* 학번 */}
      <div className="flex justify-between items-start">
      ...
      <div className="flex items-center gap-3">
        <div className="h-8 w-8 rounded-full bg-sp-surface overflow-hidden shrink-0 border border-sp-border flex items-center justify-center">
          <span className="text-xs text-sp-muted">{student?.name.charAt(0) ?? '?'}</span>
        </div>
  ...
  return (
    <div className={`flex flex-col h-full ${embedded ? '' : '-m-8'}`}>
      ...
      <div className={embedded ? '' : 'p-8 flex flex-col flex-1 min-h-0'}>
        ...
        {/* 배치 조건 모달 */}
        <SeatZoneModal open={showConstraintModal} onClose={() => setShowConstraintModal(false)} />
