...
export function ListView({ categoryFilter }: ListViewProps) {
  const { todos, categories, toggleTodo, toggleSubTask, updateTodo, archiveCompleted, addTodo } =
    useTodoStore();
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' && quickAddText.trim()) {
              void addTodo(quickAddText.trim());
              setQuickAddText('');
            }
          }}
        ...
        {Object.entries(grouped).map(([groupName, items]) => {
          ...
          return (
            <div key={groupName}>
              ...
              >
                <span
                  className="material-symbols-outlined text-icon text-sp-muted transition-transform"
...
function ListViewRow({
  ...
  return (
    <div className="border-b border-sp-border/20">
      <div
        className="flex items-center px-4 py-2.5 hover:bg-sp-surface/30 transition-colors group cursor-pointer"
        onClick={() => onEdit(todo)}
      >
        {/* 체크박스 */}
        ...
        <div className="flex-[3] min-w-0 flex items-center gap-1">
          <span
            className={`text-sm truncate ${todo.completed ? 'line-through text-sp-muted' : 'text-sp-text'}`}
          >
            {todo.text}
          </span>
          {hasSubTasks && (
          ...
          <div className="pl-12 pr-4 pb-2 bg-sp-surface/20 border-l-2 border-sp-accent/30">
            ...
            {todo.subTasks!.map((st) => (
              <div key={st.id} className="flex items-center gap-2 py-0.5">
                ...
                />
                <span
                  className={`text-xs ${st.completed ? 'line-through text-sp-muted/50' : 'text-sp-muted'}`}
                >
                  {st.text}
                </span>
              </div>
