...
const KIND_META: Record<TimetableOverrideKind, { label: string; icon: string; color: string }> = {
  ...
  custom: {
    label: '기타',
    icon: 'edit_note',
    color: 'text-sp-muted bg-sp-surface border-sp-border',
  },
};
...
export function TimetableOverridesPanel({
  open,
  onClose,
  onAddNew,
  onEdit,
}: TimetableOverridesPanelProps) {
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3">
          {filtered.length === 0 ? (
            <div className="py-12 text-center text-sp-muted text-sm">
              <span className="material-symbols-outlined text-4xl text-sp-muted/40 mb-2 block">
                event_busy
              </span>
              등록된 변동 시간표가 없습니다.
              <div className="mt-1 text-xs text-sp-muted/70">
                "+ 변동 추가" 또는 시간표 셀 우클릭으로 등록하세요.
              </div>
            </div>
          ) : (
              ...
              {filtered.map((o) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-2 flex-wrap gap-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-xs text-sp-muted font-medium">
                          ...
                          {resolvePeriodLabel(o.period, settings.periodTimes)}
                        </span>
                        <span
                    ...
                    {/* 교사 관점 */}
                    <div
                      className={`flex items-baseline gap-2 mb-1 text-xs ${appliesTeacher ? '' : 'opacity-40'}`}
                    >
                      <span
                        className={`inline-block w-10 shrink-0 text-caption font-bold ${appliesTeacher ? 'text-sky-400' : 'text-sp-muted/50'}`}
                      >
                        교사
                      </span>
                      {appliesTeacher ? (
                        <>
                          <span className="text-sp-muted">
                            ...
                            {base.teacherClassroom && (
                              <span className="ml-0.5 text-sp-muted/60">
                                @{base.teacherClassroom}
                              </span>
                            )}
                          </span>
                          ...
                          <span className="font-semibold text-sp-text">
                            {afterLabel}
                            {o.classroom && (
                              <span className="ml-0.5 text-sp-muted">@{o.classroom}</span>
                            )}
                          </span>
                        </>
                      ) : (
                        <span className="text-sp-muted/50 italic">
                          변동 없음 (그대로: {base.teacherSubject || '(빈)'})
                        </span>
                      )}
                    </div>
                    ...
                    {/* 학급 관점 */}
                    <div
                      className={`flex items-baseline gap-2 text-xs ${appliesClass ? '' : 'opacity-40'}`}
                    >
                      <span
                        className={`inline-block w-10 shrink-0 text-caption font-bold ${appliesClass ? 'text-fuchsia-400' : 'text-sp-muted/50'}`}
                      >
                        학급
                      </span>
                      {appliesClass ? (
                        <>
                          <span className="text-sp-muted">
                            ...
                            {base.classTeacher && (
                              <span className="ml-0.5 text-sp-muted/60">· {base.classTeacher}</span>
                            )}
                          </span>
                          <span className="text-sp-muted">→</span>
                    ...
                    {pair && (
                      <div className="mt-1.5 px-2 py-1 bg-blue-500/5 border border-blue-500/20 rounded text-detail text-blue-300 flex items-center gap-1">
                        ...
                        짝: {formatDateLabel(pair.date)}{' '}
                        {resolvePeriodLabel(pair.period, settings.periodTimes)}와 교체
                      </div>
                    )}

                    {/* 사유 */}
                    {o.reason && (
                      <div className="text-xs text-sp-muted/80 mt-1">사유: {o.reason}</div>
                    )}
                  </li>
                );
              })}
