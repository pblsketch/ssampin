...
import { openExternalUrl } from '@adapters/utils/openExternalUrl';
...
export function FeedbackModal({ onClose }: FeedbackModalProps) {
  ...
  function handleOpenForm() {
    // Electron 데스크톱에서는 window.open이 보안 가드로 차단되므로 openExternalUrl로 OS 브라우저에서 연다.
  ...
  return (
    <Modal isOpen onClose={onClose} title="의견을 보내주세요" srOnlyTitle size="lg">
      {/* 헤더 */}
      <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-sp-border/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-sp-text">의견을 보내주세요</h3>
            <p className="text-xs text-sp-muted mt-0.5">
              선생님의 피드백이 쌤핀을 더 좋게 만듭니다
            </p>
          </div>
        </div>
        <button
      ...
      <div className="px-6 py-5 grid grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-xs text-sp-muted mb-3 leading-relaxed">쌤핀 AI에게 바로 물어보세요!</p>
        ...
        >
          ...
          <p className="text-xs text-sp-muted mb-3 leading-relaxed">설문지로 의견을 보내주세요</p>
          <ul className="space-y-1.5 text-xs text-sp-muted mb-4 flex-1">
            <li className="flex items-center gap-1.5">
      ...
      <div className="px-6 pb-5">
        <p className="text-detail text-sp-muted text-center bg-sp-surface/50 rounded-lg py-2 px-3 border border-sp-border/30">
          사용법이 궁금하면 <span className="text-sp-accent font-medium">AI에게</span>, 자세한
          의견은 <span className="text-sp-accent font-medium">설문지</span>로 보내주시면 큰 도움이
          됩니다!
        </p>
      </div>
    </Modal>
