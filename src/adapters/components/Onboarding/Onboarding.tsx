...
export function Onboarding() {
  ...
  const handleFinish = async () => {
    ...
    const neisWithAutoSync =
      hasNeisSchool && neisGrade && neisClass
        ? {
            ...draft.neis!,
            autoSync: {
              enabled: true,
    ...
    // NEIS 학사일정 자동 동기화 설정
    const neisScheduleSettings =
      hasNeisSchool && neisScheduleEnabled
  ...
  const handleSelectSchool = useCallback(
    (school: SchoolSearchResult) => {
      ...
      // ②-B 학교알리미 식별자 매칭(네트워크·best-effort). 성공 시 ①불러오기 학교검색을 생략.
      // 실패/모호하면 schoolInfo 는 비워둔 채로 두고 ①은 수동검색으로 폴백한다(차단 없음, §13).
      void enrichSchoolOnSelect.matchSchoolInfo(school.schoolName, school.address).then((link) => {
        if (link) setDraft((prev) => ({ ...prev, schoolInfo: link }));
      });
    },
    [clearSearch],
  );
  ...
  const updatePeriod = (index: number, field: 'start' | 'end', value: string) => {
    setDraft((prev) => {
      ...
      if (field === 'start' && prev.schoolLevel) {
        const duration =
          prev.schoolLevel === 'custom' && prev.customPeriodDuration
          ...
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all ${
                i === step ? 'bg-sp-accent w-6' : i < step ? 'bg-sp-accent/50' : 'bg-sp-surface'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex-1 p-8 px-10 flex flex-col justify-center">
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-8 duration-300">
              ...
              <p className="text-sp-muted text-center mb-8">
                대시보드와 출력물에 사용될 기본 정보입니다.
              </p>
                ...
                <div className="space-y-2 col-span-2 relative">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setManualSchoolInput(!manualSchoolInput);
                        clearSearch();
                        setSchoolQuery('');
                      }}
                  ...
                  {!manualSchoolInput && draft.schoolName && !schoolQuery ? (
                    <div className="flex items-center gap-3 bg-sp-bg border border-sp-border rounded-lg px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-sp-text truncate">{draft.schoolName}</p>
                        {draft.neis?.address && (
                          <p className="text-xs text-sp-muted truncate" title={draft.neis.address}>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setSchoolQuery(draft.schoolName ?? '');
                          patch({ schoolName: '' });
                        }}
                    ...
                    <>
                      ...
                      {searchError && searchResults.length === 0 && (
                        <div className="mt-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 space-y-2">
                          <p className="text-xs text-sp-text font-medium">
                            🔍 검색 결과가 없습니다
                          </p>
                          <p className="text-xs text-sp-muted leading-relaxed">
                            유치원·학원·대안학교 등은 NEIS에 등록되어 있지 않아요. 아래 버튼으로
                            학교 정보를 직접 입력할 수 있습니다.
                          </p>
                          <button
                            type="button"
                            onClick={() => {
                              setManualSchoolInput(true);
                              clearSearch();
                              setSchoolQuery('');
                            }}
                ...
                {hasNeisSchool ? (
                  <>
                    <div className="col-span-2 flex items-start gap-2 p-2.5 rounded-lg bg-blue-500/5 border border-blue-500/15">
                      <span className="material-symbols-outlined text-blue-400 text-icon mt-0.5">
                        info
                      </span>
                      <p className="text-detail text-blue-300">
                        학년/반을 선택하면 해당 반의 <strong>학급 시간표</strong>가 자동으로
                        연동됩니다 (담임용). 건너뛰어도 나중에 설정할 수 있어요.
                      </p>
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-semibold text-sp-muted uppercase tracking-wider">
                        학년 <span className="text-sp-muted/50 normal-case">(선택)</span>
                      </label>
                      <select
                        value={neisGrade}
                        onChange={(e) => {
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-semibold text-sp-muted uppercase tracking-wider">
                        반 <span className="text-sp-muted/50 normal-case">(선택)</span>
                      </label>
                      {classListLoading ? (
                    ...
                    <div className="col-span-2 flex items-center justify-between p-3 rounded-lg bg-purple-500/5 border border-purple-500/20">
                      ...
                      >
                        <span
                          className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-lg transition-transform ${neisScheduleEnabled ? 'translate-x-5' : 'translate-x-0'}`}
                        />
                      </button>
                    </div>
                    ...
                    {neisGrade && neisClass && (
                      <div className="col-span-2 flex items-start gap-2 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20 animate-in fade-in duration-300">
                        <span className="material-symbols-outlined text-emerald-400 text-icon-md mt-0.5">
                          auto_awesome
                        </span>
                        <p className="text-xs text-emerald-300">
                          {neisGrade}학년 {neisClass}반의 학급 시간표가 대시보드에 자동으로
                          표시됩니다.
                        </p>
                      </div>
                    )}
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-8 duration-300 flex flex-col items-center">
              ...
              <p className="text-sp-muted text-center mb-6">
                학교급에 맞는 프리셋을 선택하고 시간을 수정할 수 있습니다.
              </p>
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-right-8 duration-300 flex flex-col">
              <h2 className="text-2xl font-bold text-sp-text mb-2 text-center">
                나에게 맞는 메뉴 설정
              </h2>
              <p className="text-sp-muted text-center mb-6">
                역할을 선택하면 맞춤 메뉴를 추천해 드려요.
              </p>

              {/* 역할 선택 */}
              <div className="mb-6">
                <p className="text-sm font-semibold text-sp-muted uppercase tracking-wider mb-3 text-center">
                  나의 역할 (복수 선택 가능)
                </p>
                <div className="flex justify-center gap-3">
                  {[
                    {
                      id: 'homeroom' as TeacherRoleId,
                      label: '담임교사',
                      icon: 'school',
                      desc: '학급 담임을 맡고 있어요',
                    },
                    {
                      id: 'subject' as TeacherRoleId,
                      label: '교과교사',
                      icon: 'menu_book',
                      desc: '교과 수업을 담당해요',
                    },
                    {
                      id: 'admin' as TeacherRoleId,
                      label: '관리자/부장',
                      icon: 'admin_panel_settings',
                      desc: '부장 또는 관리 업무를 해요',
                    },
                  ].map((role) => {
              ...
              {selectedRoles.length > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                  <p className="text-sm font-semibold text-sp-muted uppercase tracking-wider mb-3">
                    추천 메뉴
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span
                              className={`material-symbols-outlined text-lg ${isVisible ? 'text-sp-muted' : 'text-sp-muted/30'}`}
                            >
                              {item.icon}
                            </span>
                            <div>
                              <span
                                className={`text-sm font-medium ${isVisible ? 'text-sp-text' : 'text-sp-muted/40'}`}
                              >
                                {item.label}
                              </span>
                              {MENU_DESCRIPTIONS[item.id] && (
                                <p
                                  className={`text-xs ${isVisible ? 'text-sp-muted' : 'text-sp-muted/30'}`}
                                >
                                  {MENU_DESCRIPTIONS[item.id]}
                                </p>
                              )}
                            ...
                            {isAlwaysVisible && (
                              <span className="text-caption text-sp-muted bg-sp-surface px-1.5 py-0.5 rounded ml-1">
                                항상 표시
                              </span>
                            )}
                          </div>
                  ...
                  <p className="text-xs text-sp-muted mt-3 text-center">
                    나중에 <span className="text-sp-accent">설정 &gt; 사이드바</span>에서 언제든
                    변경할 수 있어요
                  </p>
                </div>
              )}
          ...
          {step === 6 && (
            <div className="text-center animate-in zoom-in-95 duration-500">
              ...
              <p className="text-sp-muted text-lg mb-8">
                이제 쌤핀을 사용하여 교실 업무를 더욱 스마트하게 관리하세요.
              </p>
...
function AccountLinkingStep() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-8 duration-300 flex flex-col items-center w-full">
      ...
      <p className="text-sp-muted text-center mb-6">
        다른 PC에서도 데이터를 이어 쓰려면 계정을 연결하세요. 나중에 설정에서도 할 수 있어요.
      </p>
        ...
        {isConnected ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2 rounded-lg border border-sp-border p-3">
                <span className="material-symbols-outlined text-cyan-400 text-icon-md">
                  cloud_sync
                </span>
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-sp-surface border border-sp-border p-5 space-y-3">
              <p className="text-sm font-semibold text-sp-muted uppercase tracking-wider">
                연결하면 사용할 수 있는 기능
              </p>
              <ul className="space-y-2 text-sm text-sp-muted">
                <li className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-icon text-cyan-400">
                    cloud_sync
                  </span>
                  여러 기기 간 데이터 자동 동기화
                </li>
                ...
                <li className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-icon text-emerald-400">
                    folder_open
                  </span>
                  과제수합 (Google 드라이브)
                </li>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      void cancelAuth().then(() => void startPKCEFallback());
                    }}
      ...
      {oauthError && (
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2 mt-6">
              {(oauthError.code === 'SERVER_START_FAILED' ||
                oauthError.code === 'LOCALHOST_BLOCKED') && (
                <button
                  onClick={() => {
                    setOAuthError(null);
                    void startPKCEFallback();
                  }}
                  className="rounded-lg bg-sp-accent px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-sp-accent/80"
                >
                  수동 인증으로 시도
      ...
      {showPKCEFallback && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
          onClick={() => setShowPKCEFallback(false)}
        >
          <div
            className="w-full max-w-md rounded-xl bg-sp-card border border-sp-border p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-sm text-sp-muted mb-4">
              브라우저 주소창이{' '}
              <code className="text-sp-text">http://127.0.0.1:.../callback?code=...</code>로 바뀐
              직후, 그 <strong className="text-sp-text">URL 전체</strong>(또는{' '}
              <code className="text-sp-text">code=</code> 값)을 그대로 붙여넣어 주세요.
            </p>
            {authError && (
