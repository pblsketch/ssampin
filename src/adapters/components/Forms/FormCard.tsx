...
const FORMAT_STYLES: Readonly<
  Record<FormFormat, { label: string; badge: string; iconBg: string; icon: string }>
> = {
  hwpx: {
    label: 'HWPX',
    badge: 'bg-sp-accent/20 text-sp-accent border border-sp-accent/30',
    iconBg: 'bg-gradient-to-br from-sp-accent/30 to-sp-accent/10',
    icon: 'description',
  },
  pdf: {
    label: 'PDF',
    badge: 'bg-sp-highlight/20 text-sp-highlight border border-sp-highlight/30',
    iconBg: 'bg-gradient-to-br from-sp-highlight/30 to-sp-highlight/10',
    icon: 'picture_as_pdf',
  },
  excel: {
    label: 'Excel',
...
export function FormCard({ form }: FormCardProps) {
  ...
  const handlePrint = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : '여는 데 실패했습니다', 'error');
    }
  };
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : '다운로드에 실패했습니다', 'error');
    }
  };
  ...
  const handleRename = async (e: React.MouseEvent) => {
    e.stopPropagation();
    setMenuOpen(false);
    // window.prompt 는 Electron 에서 동작하지 않아 이 버튼이 눌러도 아무 일이 없었다.
    const next = await prompt({
      title: '이름 바꾸기',
      label: '새 이름',
      initialValue: form.name,
    });
    if (next === null || next.trim() === '' || next === form.name) return;
    try {
      await updateFormAction(form.id, { name: next.trim() });
      showToast('이름을 변경했습니다', 'success');
    } catch (err) {
      showToast(err instanceof Error ? err.message : '이름 변경에 실패했습니다', 'error');
    }
  };
      ...
      >
        <div className={`aspect-[4/3] relative ${style.iconBg} flex items-center justify-center`}>
          {form.format === 'pdf' && thumbUrl ? (
            <img src={thumbUrl} alt={form.name} className="w-full h-full object-cover" />
          ) : (
