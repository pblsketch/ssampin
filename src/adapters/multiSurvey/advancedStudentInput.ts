...
function advancedInput(c, interaction) {
 ...
 function number(label,min,max,initial,change,step){var l=el('label',label),n=el('input');n.type='number';n.min=String(min);n.max=String(max);n.step=String(step||1);n.value=initial===null||initial===undefined?'':String(initial);n.setAttribute('aria-label',label);n.oninput=function(){change(n.value===''?NaN:Number(n.value));};l.appendChild(n);c.appendChild(l);return n;}
 if(t==='order'||t==='ranking'){
  ...
  // 가만히 두면 최솟값이 제출되던 것을 막는다 — 직접 적거나 막대를 움직여야 고른 것이다.
  var n=number('숫자'+(s.unit?' ('+s.unit+')':''),s.min,s.max,null,function(v){value=v;untouch();},s.step);value=null;
  var range=el('input');range.type='range';range.min=String(s.min);range.max=String(s.max);range.step=String(s.step);range.value=String(s.min);range.className='untouched';range.setAttribute('aria-label','숫자 슬라이더');range.oninput=function(){value=Number(range.value);n.value=range.value;untouch();};c.appendChild(range);
  function untouch(){range.classList.remove('untouched');}
 }else if(t==='pin'){
  ...
  s.items.forEach(function(item){
   ...
   ['x'].forEach(function(axis){
    ...
    // 가운데 값으로 미리 채우지 않는다 — 움직여야 자기 자리를 고른 것이다.
    record[key]=null;
    var box=el('div',undefined,'vl-axis');
    if(name)box.appendChild(el('p',name,'muted'));
    var out=el('output','아직 고르지 않았어요');out.className='vl-empty';box.appendChild(out);
    var range=el('input');range.type='range';range.min=String(s.min);range.max=String(s.max);range.step=String(s.step);range.value=String(start);range.className='untouched';
    range.setAttribute('aria-label',(item.text?item.text+' · ':'')+(name?name+' · ':'')+lo+' 에서 '+hi+' 사이');
    range.oninput=function(){record[key]=Number(range.value);out.textContent=range.value;out.className='';range.classList.remove('untouched');};
 ...
 var read=function(){
  ...
  if(t==='numeric'){if(value===null||!Number.isFinite(value)){read.hint='숫자를 적거나 막대를 움직여 골라 주세요.';return null;}return value>=s.min&&value<=s.max?value:null;}
  if(t==='valueline'&&Object.values(record).some(function(v){return v===null;})){read.hint='막대를 움직여 내 자리를 골라 주세요.';return null;}
