 ...
 *
 * 검색도 여기서 한다(2026-08-19). **잠겨 있으면 검색어를 무시한다** — 내용을 안 보여
 * 주면서 걸러 주기까지 하면, 남는 개수만으로 "그 낱말이 든 메모가 몇 개인지"가 새어
 * 나간다. 가리는 것과 세는 것을 함께 막아야 가린 것이 된다.
 */
...
/**
 * 검색어에 걸리는지 본다. 대소문자는 무시한다(한글은 영향이 없고, 영어 메모에서만 의미가 있다).
 * 제목처럼 쓰는 첫 줄만이 아니라 **본문 전체**를 본다 — 찾는 말이 아래쪽에 있다고
 * 안 나오면, 있는 메모를 없다고 말하는 셈이 된다.
 */
export function matchesSidePinMemoQuery(content: string, query: string): boolean {
  return content.toLowerCase().includes(query.toLowerCase());
}

/**
 * 검색 중일 때 보여줄 미리보기 — **걸린 줄**을 보여준다.
 *
 * 그냥 둘째·셋째 줄을 보여주면, 찾는 말이 안 보이는 메모가 결과에 떠서 "이건 왜 나왔지"가
 * 된다. 어디가 걸렸는지 보여야 고를 수 있다. 걸린 줄이 제목으로 쓴 첫 줄이면
 * 평소 미리보기로 되돌린다 — 같은 줄을 위아래로 두 번 보여줄 이유가 없다.
 */
export function deriveSidePinMemoMatchPreview(content: string, query: string): string {
  const lines = meaningfulLines(content);
  const hitIndex = lines.findIndex((line) => matchesSidePinMemoQuery(line, query));
  if (hitIndex <= 0) return deriveSidePinMemoPreview(content);
  return lines.slice(hitIndex, hitIndex + SIDE_PIN_MEMO_PREVIEW_LINES).join('\n');
}
...
export function selectSidePinMemos(input: SelectSidePinMemosInput): readonly SidePinMemoListItem[] {
  ...
  // 잠겨 있으면 검색어를 버린다(머리말 참조). 앞뒤 공백만 있는 말도 검색으로 치지 않는다.
  const query = input.locked ? '' : (input.query ?? '').trim();

  const active = input.memos.filter(
    (memo) => !memo.archived && (query === '' || matchesSidePinMemoQuery(memo.content, query)),
  );
  ...
  return shown.map((memo) => ({
    ...
    preview: input.locked
      ? ''
      : query === ''
        ? deriveSidePinMemoPreview(memo.content)
        : deriveSidePinMemoMatchPreview(memo.content, query),
    color: memo.color,
