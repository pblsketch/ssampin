...
export function registerOwnAiHandlers(deps: OwnAiHandlerDeps): void {
   ...
   * 방금 확인한 상태를 들고 있는다.
   *
   * ★확인 한 번에 CLI 를 두 번 띄운다(버전·로그인). 창을 숨겨도 프로세스는 뜨고,
   * 창이 잠깐 깜빡이기도 한다(오너 신고 2026-09-06). **로그인은 한 번 하면 유지되는데
   * 우리가 자꾸 확인하는 것**이 문제였다 — 5초 캐시로는 창을 옮길 때마다 다시 띄웠다.
   * 10분으로 늘린다. 로그인·로그아웃·모델 변경처럼 **우리가 바꾼 순간**에는 캐시를 버리므로
   * 늦게 반영될 걱정은 없다. 카드의 [다시 확인]도 캐시를 건너뛴다.
   */
