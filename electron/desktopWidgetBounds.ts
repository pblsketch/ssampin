/**
 * 위젯 윈도우 화면 경계 제한(Clamping) 및 가시성 검증 유틸리티 (Electron 메인 프로세스용).
 *
 * `src/domain/services/screenBoundsClamp.ts`의 도메인 로직과 100% 미러링되며,
 * Electron main process의 rootDir 제약을 만족한다.
 * 미러 동기화는 `electron/desktopWidgetBounds.mirror.test.ts`로 보장한다.
 */

export interface WidgetScreenRect {
  readonly x: number;
  readonly y: number;
...
export interface WidgetScreenClampOptions {
  /**
...
export const DEFAULT_WIDGET_SCREEN_CLAMP_OPTIONS: WidgetScreenClampOptions = {
...
export function clampWidgetBoundsToWorkArea(
  ...
  options?: Partial<WidgetScreenClampOptions>,
): WidgetScreenRect {
  const minHeaderHeight =
    options?.minVisibleHeaderHeight ?? DEFAULT_WIDGET_SCREEN_CLAMP_OPTIONS.minVisibleHeaderHeight;
  const minWidth = options?.minVisibleWidth ?? DEFAULT_WIDGET_SCREEN_CLAMP_OPTIONS.minVisibleWidth;
 ...
 */
export function getIntersectionArea(rectA: WidgetScreenRect, rectB: WidgetScreenRect): number {
 ...
 */
export function findBestWorkAreaForBounds<T extends WidgetScreenRect>(
  bounds: WidgetScreenRect,
