...
- **결정**:
  ...
  2. **필요 시 확장(340×480)** — 말풍선·팝오버·메뉴·코치마크가 필요할 때만 main 이 창을 키움. 핀의 화면 위치는 불변(확장 방향으로 창이 자람). 기하는 `electron/iconWindowGeometry.ts` 순수 함수(4방향 anchor 플립, 화면 여유 기준)로 분리해 단위 테스트.
...
- **함께 결정**: 전역 `body.ssampin-icon-popup * { background: transparent !important }` 를 구조 요소 한정으로 축소(말풍선/팝오버 카드 배경 생존). 단일 클릭 = 오늘 요약 팝오버(창 복원은 더블클릭·팝오버 버튼으로) — 아이콘 모드를 "닫기 방식"에서 "그 자리 비서"로 재정의. 핀 이름 `PIN_NAME='핀이'` 단일 상수.
