...
## ADR-075: 상담 예약의 수업 시간 제외는 날짜마다 따로 계산한다 — 첫 날짜 하나로 나머지를 짐작하지 않는다
...
1. **편집 모달에는 시간표 연동이 없다.** 만든 뒤 날짜를 고치면 시간표를 다시 못 본다.
   생성 시점이 맞아지면 사고는 막히므로 별도 작업으로 미뤘다.
2. **만든 뒤의 자동 재계산은 정규 시간표를 여전히 안 본다.** `buildBusyPeriods` 는 학교
   행사와 시간표 **임시 변경**만 본다. 상담 일정을 만든 뒤에 정규 시간표가 바뀌면 앱은
   모른다. 이걸 고치려면 이미 예약된 슬롯을 나중에 차단 후보로 만들게 되어 ADR-060 과
   함께 다시 따져야 한다.
