...
- **요청**: "시간표 화면 새로고침처럼 위젯 새로고침에서도 컴시간 변동을 확인하고 싶다"(2026-08-11 피드백, 챗봇 에스컬레이션). 사실 확인: 새로고침(`triggerRefreshAll`)은 저장 파일을 다시 읽을 뿐이고 서버 조회가 **0회**였다 — 챗봇 답변이 맞았다.
- **결정**: 변동 확인은 **새로고침 버튼의 onClick 에만** 얹고, `useWidgetRefresh` 와는 **다른 이벤트**(`ssampin:widget-check-timetable`)로 분리한다. 컴시간·압핀 중 켜져 있는 것만 확인하고, 연동을 쓰지 않으면 조회도 안내도 하지 않는다(침묵).
- **왜 분리가 핵심인가**: `useWidgetRefresh` 는 버튼뿐 아니라 **5분 타이머와 창 활성화**에도 같은 콜백을 부른다. 여기에 얹었다면 위젯으로 상주하는 사용자 전원이 comci.net·sgpap.com 을 종일 폴링했을 것이다(하루 1회 원칙 위반·차단 위험). 이벤트가 갈라져 있다는 사실 자체를 소스 계약 테스트로 고정했다.
- **새로고침 버튼은 두 곳에 있다**: 위젯 창 헤더(`Widget.tsx`)와 메인 창 대시보드 헤더(`DashboardHeader.tsx`). 위젯만 고쳤다가 대시보드에서 눌렀을 때 무반응이라는 실사용 보고를 받았다(2026-08-12). 두 곳이 같은 훅(`useTimetableChangeCheck`)을 쓰도록 통일하고, **"두 화면 모두 확인을 부른다"를 소스 계약 테스트로 고정**했다.
- **표현과 판정의 분리**: 위젯 창에는 `ToastContainer` 가 없어(App.tsx `WidgetApp`) 확인 함수의 토스트가 **한 줄도 보이지 않는다**. 그래서 `checkComciganTimetableChange`/`checkAppinTimetableChange` 가 판정 결과를 반환하고(`silent` 옵션), 안내는 창이 직접 그린다 — 메인 창은 토스트, 위젯은 하단 배너(ADR-043과 같은 계열의 함정).
...
- **눌린 자리에 즉시 반응을 준다**: 결과 토스트는 화면 반대편 구석에서 3초만 떠서, 버튼 근처에 아무 변화가 없으면 "눌러도 무반응"으로 읽힌다(실사용 보고). 확인 중에는 아이콘을 돌리고, **쿨다운·예외처럼 배너가 없는 화면에서 조용히 끝나던 분기도 토스트로 표면화**한다.
...
- **회귀**: `useTimetableChangeCheck.test.ts` 20건(자동 경로 무조회·침묵·쿨다운·연타·로드 선행 순서·소스 계약 2건) + `navigationTarget.test.ts` 5건.
