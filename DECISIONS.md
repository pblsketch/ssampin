...
- [ADR-087 · 온라인 교무실 제출 과제: 취합은 부장만 하는 일이 아니고, 이름은 필요한 사람에게만 (2026-09-07)](docs/03-decisions/ADR-087.md)
...
- [ADR-092 · 실기기 대행 QA 결함 10건의 처리 방향 — 기본 선택·분량 조절 대상·설명문 판정·[중단]·낭독 한국어 (2026-09-08)](docs/03-decisions/ADR-092.md)
- [ADR-093 · 생활기록부 화면 단순화 — 집중 보기가 기본, 보조 공간은 하나, 실행 상태는 스토어가 든다 (2026-09-08)](docs/03-decisions/ADR-093.md)
- [ADR-094 · 생활기록부 초안은 교사 평가로 연다 — 평가가 맨 앞, 첫 문장은 '~하는 학생임.' (2026-09-08)](docs/03-decisions/ADR-094.md)
- [ADR-095 · 상담·설문 교사 명단 조회는 Google 신원으로 연다 — 소유자는 만들 때 서버가 박고, 옛 열쇠 경로는 기한으로 닫는다 (2026-09-09, v2.5.1 출시 완료 · S6 옛 RPC 권한 회수 잔여)](docs/03-decisions/ADR-095.md)
- [ADR-096 · 학생 자기평가서는 과제 수합 위에 올린다 — 거울 카드로 받고, 교사가 고른 것만 근거가 된다 (2026-09-08)](docs/03-decisions/ADR-096.md)
- [ADR-097 · 학생 번호는 배열 위치가 아니라 명렬표의 실제 출석번호다 — 제출물 연결도 소속을 먼저 본다 (2026-09-08, v2.5.1 출시 완료)](docs/03-decisions/ADR-097.md)

- [ADR-098 · 쌤도구를 별도 창으로 띄운다 — 상태는 "멈추고 담아 넘기고", 소유자는 언제나 한 창 (2026-09-09, v2.5.1 출시 완료)](docs/03-decisions/ADR-098.md)

- [ADR-099 · 작성 방식은 선생님이 고른다 — 공통 규정과 작성 구성을 가르고, 순서는 요청서가 정한다 (2026-09-09, 서버 판본 3 적용 완료 · v2.5.1 출시 완료)](docs/03-decisions/ADR-099.md)

- [ADR-100 · 학급 목록에 수업 요일·교시를 보여준다 — 근거는 주 단위로 하나만, 자리는 행 전체 폭 (2026-09-09, v2.5.1 출시 완료)](docs/03-decisions/ADR-100.md)
...
- [ADR-103 · 생기부 근거 정리의 기본 보기는 흐름 그래프다 — 장면 배열이 곧 작성 구성이다 (2026-09-10, P0~P5 구현·게이트 4종 완료 · 미커밋 · v2.5.2 예정)](docs/03-decisions/ADR-103.md)
- [ADR-104 · 흐름 보기에 그래프 조망을 덧입힌다 — 개요 지도·이어진 흐름 섹션·범례·사이 삽입·되돌리기·AI 요청 칸, 화면 낱말은 주제/흐름(서사 금지) (2026-09-10, 구현·게이트 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-104.md)
- [ADR-105 · 생기부 한도는 선생님이 수업반·영역마다 직접 정하고, 넘어도 저장은 막지 않고 색으로 알린다 — 규정 표는 덧씌운다, ADR-072 결정 5 의 저장 거부 대체 (2026-09-11, 구현·게이트 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-105.md)
- [ADR-106 · 근거 정리의 기본 보기는 근거 지도다 — 근거끼리 직접 잇고(분기·합류), 연결은 근거 파일·위치는 기기·글 순서는 장면으로 분리, 요청서에 「근거 사이 연결」 실림, 상단 단추 3개 (2026-09-11, 1차 구현·검사 완료 · 미커밋 · v2.5.2, ADR-103 결정 1·ADR-104 결정 1~2 부분 대체)](docs/03-decisions/ADR-106.md)
- [ADR-107 · 장면 체계는 근거 지도 안으로 — 장면 열(x = 장면 차례)·주제 사이 이음말 화살표 라벨·오른쪽 상세 다섯 모드(근거·연결·장면·주제·주제 이음), 흐름 보기 화면 제거(옛 설정 flow 는 지도로 읽음), ADR-103 결정 1·ADR-104 화면 대체 (2026-09-11, 구현·검사 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-107.md)
- [ADR-108 · 근거 지도의 화살표는 근거 사이가 아니라 **장면 사이**다 — 열 머리 둘째 줄에 앞 장면→이 장면 화살표와 이음말(`NarrativeScene.leadIn`, 요청서 「앞 장면에서 이어짐」), 근거 연결 손잡이·[연결하기]·연결 편집 공간 제거, 장면 없는 묶음은 날짜순 흐름, [카드 위치 정돈]은 옮긴 카드가 있을 때만 (2026-09-11 오너 실기기 제보, 구현·검사 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-108.md)
- [ADR-109 · 비어 있는 평가 장면은 건너뛰지 않는다 — 요청서가 「근거 전체를 종합해 교사 판단을 쓰라」로 바꿔 말하고(다른 빈 장면은 그대로 건너뜀), AI 장면 배치는 종합 판단 기록이 있을 때만 평가에 놓고 없으면 비움, 지도·장면 상세·AI 제안이 "비워 두어도 된다"고 안내, 빈 번호 칸 파서 수정 (2026-09-11, 구현·검사 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-109.md)
- [ADR-110 · 생기부 초안 분량 — 요청서가 늘 분량 줄을 싣고(1층 규정에는 바이트 한도가 없었다), 목표를 10% 넘게 넘기면 앱이 한 번 줄여 두 판을 다 남기며, 분량 조절의 2차는 목표를 밀지 않고 1차 결과에서 이어 간다(뺄 양은 범위·문장 수로), ADR-088 재조정 방법 대체 (2026-09-11, 구현·검사 완료 · 미커밋 · v2.5.2)](docs/03-decisions/ADR-110.md)
- [ADR-111: 근거 지도 가지형 배치·직접 편집·AI 맥락 제안과 되돌리기 (2026-09-13)](docs/03-decisions/ADR-111.md)
- [ADR-112: 연결점으로 장면 차례 편집·근거 다중 참조·연결별 메모 (2026-09-13)](docs/03-decisions/ADR-112.md)
- [ADR-113: 지도 장면 차례 검증·생성 본문 가운데점의 쉼표 정규화 (2026-09-13)](docs/03-decisions/ADR-113.md)
- [ADR-114: Codex Terra·Luna의 근거 중심 상세 서술 기본 적용 (2026-09-13)](docs/03-decisions/ADR-114.md)

- [ADR-115: 전과목 성적 다중 행 인식과 확인 후 파일 단위 교체 (2026-09-13)](docs/03-decisions/ADR-115.md)
- [ADR-116: 뼈대 선택창 닫기·직접 뼈대 만들기·주제별 장면 추가 (2026-09-13)](docs/03-decisions/ADR-116.md)

- [ADR-117: 지도 미분류 접근과 근거 보존 (2026-09-13)](docs/03-decisions/ADR-117.md)

- [ADR-118: 근거 관리 실행 취소·다시 실행과 Ctrl+Z/Ctrl+X (2026-09-13)](docs/03-decisions/ADR-118.md)

- [ADR-119: 여러 학생의 근거 지도 제안은 대상·확인 간격·뼈대를 나누고 검토 후 적용한다 (2026-09-13, 로컬 구현·미배포)](docs/03-decisions/ADR-119.md)

- [ADR-122: 오늘 기록 위젯을 담임+수업반 학생 기록 허브로 넓히고, 저장 위치는 항상 선생님이 고른다 (2026-09-15 결정, v2.5.3 출시)](docs/03-decisions/ADR-122.md)

- [ADR-123: 컴시간 이번 주 보강·교체를 알림 대신 그 주 날짜의 변동 시간표로 자동 등록한다 (2026-09-18, ADR-091 3항 대체, v2.5.3 출시)](docs/03-decisions/ADR-123.md)

- [ADR-124: 멀티설문 v2 라이브에 입장 코드를 다시 보여 준다 — 2026-06-12 "QR+URL 전용" 결정 대체 (2026-09-18, 구현·자동검사 완료 · 실기 미검증 · 미출시)](docs/03-decisions/ADR-124.md)
- [ADR-125 · 참여교실은 문항 유형으로 만들고 같은 화면으로 체험한다](docs/03-decisions/ADR-125.md) — 채택 · 2026-09-21
- [ADR-126 · 참여교실 문항 유형을 합치고(가치수직선·아이디어), 학생별 응답은 교사 화면에만 둔다](docs/03-decisions/ADR-126.md) — 채택 · 2026-09-21 · 미출시
- [ADR-127 · 참여교실 세 화면의 시각 언어 — 유형 5무리 색·아이콘, 결과 그림 두 거리(compact/display), 보기 글자 통일](docs/03-decisions/ADR-127.md) — 채택 · 2026-09-21 · 미출시
- [ADR-128 · 쌤도구 '참여교실' → '퀴즈·설문·토론'. 이름은 상수 한 곳에서만 정한다(목록이 세 군데라 갈라져 있었다)](docs/03-decisions/ADR-128.md) — 채택 · 2026-09-21 · 미출시
- [ADR-129 · 문항 유형을 퀴즈·토론·설문으로 나눠 보여 주되 모드로 가두지 않는다(ADR-127 1절 대체, ADR-125 유지)](docs/03-decisions/ADR-129.md) — 채택 · 2026-09-21 · 미출시
- [ADR-130 · 2×2 매트릭스를 따로 두고 가치수직선은 한 줄만 남긴다(ADR-126 일부 되돌림 · 옛 2축 평가는 새로 만들 수 없음)](docs/03-decisions/ADR-130.md) — 채택 · 2026-09-21 · 미출시
- [ADR-131 · 퀴즈·설문·토론 진행을 세 축으로 나눈다 — 응답 마감 · 결과 공개 · 정답 공개(교실 화면에는 공개 전 정답을 보내지 않는다)](docs/03-decisions/ADR-131.md) — 채택 · 2026-09-21 · 미출시
- [ADR-132 · 학생 입장은 인터넷 주소가 정식 경로, 같은 Wi-Fi 주소는 경고와 함께 보조로만(준비 중에는 주소를 안 보여 주고 활동 시작을 막는다)](docs/03-decisions/ADR-132.md) — 채택 · 2026-09-22 · 미출시
- [ADR-133 · 퀴즈·설문·토론과 겹치는 옛 도구 5종(객관식 설문·주관식 설문·워드클라우드·가치수직선 토론·신호등 토론)을 목록에서 내린다. 실시간 담벼락은 남긴다](docs/03-decisions/ADR-133.md) — 채택 · 2026-09-22 · 미출시
