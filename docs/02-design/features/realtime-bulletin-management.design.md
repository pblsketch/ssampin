...
depends_on: docs/01-plan/features/realtime-bulletin-management.plan.md
---
...
> **Planning Doc**: [realtime-bulletin-management.plan.md](../../01-plan/features/realtime-bulletin-management.plan.md)
...
- **기존 코드 비파괴**: realtime-bulletin BETA의 5개 IPC 채널(`start/stop/tunnel-*/student-submitted/connection-count`)과 도메인 타입(`RealtimeBulletinBoard/Post/Column`)은 **1줄도 건드리지 않는다**. 관리 레이어는 전부 additive.
...
| domain/entities | `RealtimeBulletinPreset.ts` | 🆕 | | | 신규 |
| domain/entities | `RealtimeBulletinSnapshot.ts` | | 🆕 | | 신규 |
...
| domain/ports | `IRealtimeBulletinSnapshotPort.ts` | | 🆕 | | 신규 |
| domain/repositories | `IRealtimeBulletinPresetRepository.ts` | 🆕 | | | 신규 |
| domain/repositories | `IBulletinBoardRepository.ts` | | | 🆕 | 신규 |
| domain/rules | `realtimeBulletinRules.ts` | ⚡추가 | ⚡추가 | ⚡추가 | 신규 함수 add-only |
| usecases/realtimeBulletin | `ManagePresets.ts` | 🆕 | | | 신규 |
| usecases/realtimeBulletin | `SaveBulletinSnapshot.ts` | | 🆕 | | 신규 |
| usecases/realtimeBulletin | `LoadBulletinSnapshot.ts` | | 🆕 | | 신규 |
| usecases/realtimeBulletin | `ListBulletinSnapshots.ts` | | 🆕 | | 신규 |
| usecases/realtimeBulletin | `DeleteBulletinSnapshot.ts` | | 🆕 | | 신규 |
| usecases/realtimeBulletin | `ManageBulletinBoard.ts` | | | 🆕 | 신규 |
| usecases/realtimeBulletin | `StartBulletinBoardSession.ts` | | | 🆕 | 신규 (M2 snapshot 로드 + M3 엔티티 결합) |
| adapters/repositories | `JsonRealtimeBulletinPresetRepository.ts` | 🆕 | | | 신규 |
| adapters/repositories | `FileBulletinBoardRepository.ts` | | | 🆕 | 신규 |
| adapters/stores | `useToolPresetStore.ts` | ⚡수정 | | | 신규 type 처리 |
| adapters/stores | `useRealtimeBulletinPresetStore.ts` | 🆕 | | | 신규 (선택: useToolPresetStore 단독 확장도 가능. §8.2 결정 기록) |
| adapters/stores | `useRealtimeBulletinSessionStore.ts` | | 🆕 | | 신규 (런타임 autosave 상태) |
| adapters/stores | `useRealtimeBulletinBoardStore.ts` | | | 🆕 | 신규 |
| adapters/components/Tools | `ToolRealtimeBulletin.tsx` | ⚡수정 | ⚡수정 | ⚡수정 | viewMode 확장 |
| adapters/components/Tools/RealtimeBulletin | `PresetPicker.tsx` | 🆕 | | | 신규 |
| adapters/components/Tools/RealtimeBulletin | `PresetManagerModal.tsx` | 🆕 | | | 신규 |
| adapters/components/Tools/RealtimeBulletin | `ResumeSessionBanner.tsx` | | 🆕 | | 신규 |
| adapters/components/Tools/RealtimeBulletin | `EndSessionDialog.tsx` | | 🆕 | | 신규 |
| adapters/components/Tools/RealtimeBulletin | `BulletinBoardList.tsx` | | | 🆕 | 신규 |
| adapters/di | `container.ts` | ⚡수정 | ⚡수정 | ⚡수정 | Phase별 의존성 조립 추가 |
| infrastructure/realtimeBulletin | `FileBulletinSnapshotPersistence.ts` | | 🆕 | | 신규 |
| infrastructure/realtimeBulletin | `FileBulletinBoardPersistence.ts` | | | 🆕 | 신규 |
| electron/ipc | `realtimeBulletin.ts` | | ⚡추가 | ⚡추가 | IPC 채널 add-only |
...
domain/                                   (순수 TS)
  ↑
usecases/realtimeBulletin/                (domain만 import, M1~M3)
  ↑
adapters/                ──→  infrastructure/realtimeBulletin/
...
검증 (Phase별):
M1:
- [x] RealtimeBulletinPreset entity → 0 외부 의존
- [x] ManagePresets usecase → IRealtimeBulletinPresetRepository만
- [x] JsonRealtimeBulletinPresetRepository → window.electronAPI.readData/writeData만
- [x] PresetPicker/Modal → usecase/store만 참조

M2:
- [x] RealtimeBulletinSnapshot entity → 0 외부 의존 (기존 RealtimeBulletinPost 재사용)
- [x] IRealtimeBulletinSnapshotPort → 포트만 정의
- [x] SaveBulletinSnapshot usecase → port만 주입
- [x] FileBulletinSnapshotPersistence → fs/path (infrastructure OK)
- [x] electron/ipc/realtimeBulletin.ts → infrastructure 호출 OK
...
│                                                                       │
│  ToolRealtimeBulletin.tsx                                             │
│   (viewMode: 'list' → 'create' → 'running' → 'results')                │
│    │                                                                  │
│    ├─ [M3] BulletinBoardList.tsx — 보드 목록 (useRealtimeBulletinBoardStore)
│    ├─ [M1] CreateView                                                 │
...
│    │    └─ [M2] useRealtimeBulletinSessionStore (autosave state)      │
│    └─ ResultView                                                      │
│                                                                       │
...
│                                                                       │
│  usecases/realtimeBulletin/*  (domain만 import)                       │
│                                                                       │
│  infrastructure/realtimeBulletin/                                     │
...
```typescript
// domain/entities/RealtimeBulletinPreset.ts  [신규]
import type { RealtimeBulletinLayoutMode } from './RealtimeBulletin';

export interface RealtimeBulletinPreset {
  readonly id: string;
...
export type ToolPresetType = TextListPresetType | 'realtime-bulletin';
...
export interface RealtimeBulletinPresetEntry extends ToolPresetBase {
...
export type ToolPreset = TextListPreset | RealtimeBulletinPresetEntry;
...
**호환성 전략**: 기존 `items: string[]`만 있는 엔트리는 자동으로 `type: 'roulette' | 'random' | 'wordcloud'`로 분류됨(discriminated union이라 TS가 강제). v1 파일을 읽어도 `realtime-bulletin` 엔트리는 없어서 무해. v2 파일을 구버전 앱이 읽으면 `'realtime-bulletin'` 타입은 switch default로 fall through (구버전 코드가 이미 `switch(type)`에서 unknown default 처리 필요 — §8.3 마이그레이션에서 가드).
...
export type RealtimeBulletinSessionState = 'running' | 'paused' | 'ended';

export interface RealtimeBulletinSnapshot {
  ...
  readonly sessionState: RealtimeBulletinSessionState;
  readonly tunnelActive: boolean;          // 저장 시점에 터널 열려있었는지 (UI 힌트)
}
```

**설계 포인트**:
- `RealtimeBulletinPost[]`는 기존 타입 그대로 재사용 → 추가 타입 폭발 없음
...
import type { RealtimeBulletinColumn, RealtimeBulletinLayoutMode } from './RealtimeBulletin';
...
```typescript
// adapters/stores/useRealtimeBulletinPresetStore.ts [신규]
interface RealtimeBulletinPresetStore {
  presets: readonly RealtimeBulletinPresetEntry[];
  loaded: boolean;
  ...
  }) => Promise<RealtimeBulletinPresetEntry>;
...
**결정**: 별도 `useRealtimeBulletinPresetStore` **신규 생성** (기존 `useToolPresetStore` 확장이 아니라).

**이유**:
- `TextListPreset.items: string[]`과 `RealtimeBulletinPresetEntry.columnTitles` 의미 차이가 크다
...
interface PresetPickerProps {
  ...
  readonly presets: readonly RealtimeBulletinPresetEntry[];
  readonly onSelect: (preset: RealtimeBulletinPresetEntry | null) => void;
  readonly onOpenManager: () => void;
}
```

**동작**:
- `onSelect(preset)` 호출 시 부모(`ToolRealtimeBulletin.tsx`)가 `setTitle(preset.titleTemplate ?? '')`, `setLayoutMode(preset.layoutMode)`, `setColumnInputs([...preset.columnTitles])` 실행
...
|---|---|---|---|---|
| `realtime-bulletin:save-snapshot` | R→M | `{ snapshot: RealtimeBulletinSnapshot }` | `{ savedAt: number }` | 스냅샷 저장 (debounced + interval + 수동) |
| `realtime-bulletin:load-snapshot` | R→M | `{ id: string }` | `RealtimeBulletinSnapshot \| null` | 재개 시 로드 |
| `realtime-bulletin:list-snapshots` | R→M | `{}` | `Array<{ id: string; title: string; savedAt: number; sessionState: RealtimeBulletinSessionState }>` | 재개 가능 목록 |
| `realtime-bulletin:delete-snapshot` | R→M | `{ id: string }` | `{ ok: true }` | 스냅샷 삭제 |

**네이밍 규칙**: `realtime-bulletin:*` prefix 고수 (기존 5채널과 동일). collab-board의 `collab-board:*`와는 완전 분리.

### 5.2 preload 브릿지 확장

```typescript
// electron/preload.ts [⚡추가]
// 기존 realtime-bulletin 섹션에 add-only:
realtimeBulletinSaveSnapshot: (
  snapshot: RealtimeBulletinSnapshot,
): Promise<{ savedAt: number }> =>
  ipcRenderer.invoke('realtime-bulletin:save-snapshot', { snapshot }),

realtimeBulletinLoadSnapshot: (
  id: string,
): Promise<RealtimeBulletinSnapshot | null> =>
  ipcRenderer.invoke('realtime-bulletin:load-snapshot', { id }),

realtimeBulletinListSnapshots: (): Promise<Array<{
  id: string;
  title: string;
  savedAt: number;
  sessionState: RealtimeBulletinSessionState;
}>> =>
  ipcRenderer.invoke('realtime-bulletin:list-snapshots', {}),

realtimeBulletinDeleteSnapshot: (
  id: string,
): Promise<{ ok: true }> =>
  ipcRenderer.invoke('realtime-bulletin:delete-snapshot', { id }),
```
...
app.on('before-quit', () => {
  ...
  try {
    endActiveBulletinSessionSync();                    // 🆕 realtime-bulletin M2
  } catch (e) { console.error('[bulletin sync save]', e); }
  // ... 기존 analytics flush 유지
});
```

**`endActiveBulletinSessionSync()` 구현 위치**: `electron/ipc/realtimeBulletin.ts`에서 export. 내부 구현:
...
| 트리거 | 주기 | 구현 |
|---|---|---|
| setInterval 안전망 | 30초 | `useRealtimeBulletinSessionStore`에서 `setInterval` 훅, running 뷰 진입 시 start / 종료 시 clear |
...
진입 배너: `ToolRealtimeBulletin.tsx`의 `viewMode === 'create'` 진입 시 최상단에 렌더링. 재개할 스냅샷이 1개만 있으면 단일 "이어서 열기" 배너, 여럿이면 리스트.
...
[M2 신규 Flow]
(A) 진입 시 재개:
  ToolRealtimeBulletin mount
    → electronAPI.realtimeBulletinListSnapshots()
...
(B) 진행 중 autosave:
  posts/columns 변경 시 debounce(3s) 트리거
    → electronAPI.realtimeBulletinSaveSnapshot(snapshot)
...
| 채널 | 방향 | Payload | Response | 용도 |
|---|---|---|---|---|
| `realtime-bulletin:list` | R→M | `{}` | `BulletinBoard[]` | 보드 목록 |
| `realtime-bulletin:create` | R→M | `{ name: string; presetId?: string; layoutMode; columns; title? }` | `BulletinBoard` | 신규 보드 |
| `realtime-bulletin:rename` | R→M | `{ id: string; name: string }` | `BulletinBoard` | 이름 변경 |
| `realtime-bulletin:update` | R→M | `{ id: string; patch: Partial<{title;layoutMode;columns}> }` | `BulletinBoard` | 설정 변경 (카드 X) |
| `realtime-bulletin:delete` | R→M | `{ id: string }` | `{ ok: true }` | 삭제 (스냅샷도 함께) |
| `realtime-bulletin:duplicate` | R→M | `{ id: string; newName: string }` | `BulletinBoard` | 복제 |
| `realtime-bulletin:start-session` | R→M | `{ id: string }` | `{ board: BulletinBoard; snapshot: Snapshot \| null }` | 세션 진입 (M2 snapshot 로드 연계) |
| `realtime-bulletin:get-active` | R→M | `{}` | `{ id: string; startedAt: number } \| null` | 현재 활성 보드 조회 (탭 재진입 시) |
...
**활성 세션 제약**: 쌤핀 전체에서 터널은 싱글턴 유지. `realtime-bulletin:start-session`은 `collab-board`가 활성이거나 다른 Live 도구가 활성이면 에러 반환 → UI Toast.
...
bulletinBoards: {
  list: (): Promise<BulletinBoard[]> => ipcRenderer.invoke('realtime-bulletin:list'),
  create: (args) => ipcRenderer.invoke('realtime-bulletin:create', args),
  rename: (args) => ipcRenderer.invoke('realtime-bulletin:rename', args),
  update: (args) => ipcRenderer.invoke('realtime-bulletin:update', args),
  delete: (args) => ipcRenderer.invoke('realtime-bulletin:delete', args),
  duplicate: (args) => ipcRenderer.invoke('realtime-bulletin:duplicate', args),
  startSession: (args) => ipcRenderer.invoke('realtime-bulletin:start-session', args),
  getActive: () => ipcRenderer.invoke('realtime-bulletin:get-active'),
},
```

### 7.3 UI — BulletinBoardList

**진입**: `ToolRealtimeBulletin.tsx`에서 `viewMode === 'list'`일 때 렌더링 (M3부터 이게 초기 viewMode).
...
```typescript
// ToolRealtimeBulletin.tsx 내부 상태
...
|---|---|---|
| `RealtimeBulletinPreset`, `RealtimeBulletinSnapshot`, `BulletinBoard` | Domain/Entities | M1/M2/M3 |
| `BulletinBoardId`, brand 타입 | Domain/ValueObjects | M3 |
| `IRealtimeBulletinPresetRepository`, `IBulletinBoardRepository` | Domain/Repositories | M1/M3 |
| `IRealtimeBulletinSnapshotPort` | Domain/Ports | M2 |
| `realtimeBulletinRules.ts` 신규 함수 (preset 검증 등) | Domain/Rules | M1~M3 |
| `ManagePresets`, `SaveBulletinSnapshot`, `ManageBulletinBoard` 등 | Application/Usecases | M1~M3 |
| `JsonRealtimeBulletinPresetRepository`, `FileBulletinBoardRepository` | Adapters/Repositories | M1/M3 |
| `useRealtimeBulletinPresetStore`, `...SessionStore`, `...BoardStore` | Adapters/Stores | M1~M3 |
...
| IPC handlers in `electron/ipc/realtimeBulletin.ts` | Electron Main (infra 등가) | M2/M3 |
...
- [ ] electron/ipc/realtimeBulletin.ts는 usecases + infrastructure만 import (adapters 금지)
- [ ] 기존 realtime-bulletin 5채널 로직은 1줄도 수정되지 않았는가 (diff 검증)
```
...
```typescript
// M1 추가
export const realtimeBulletinPresetRepository = new JsonRealtimeBulletinPresetRepository();

// M2 추가
export const realtimeBulletinSnapshotPersistence = new FileBulletinSnapshotPersistence(/* electronAPI IPC */);

// M3 추가
export const bulletinBoardRepository = new FileBulletinBoardRepository(realtimeBulletinSnapshotPersistence);
```
...
- [x] **학생 제출 검증은 기존 `realtimeBulletin.ts` validateNickname/normalizeLink 재사용** — 이 설계는 학생 입력 경로 변경 없음.
- [x] **IPC Payload 검증**: 모든 handler는 `args` 필드 타입 검증 후 동작. `realtime-bulletin:save-snapshot`의 `snapshot.posts` 개수 상한(1000) 체크 → DoS 방지.
...
| Type | 대상 | Tool | Phase |
|---|---|---|---|
| Unit | `realtimeBulletinRules.ts` 신규 함수 (preset 검증, snapshot 직렬화 스키마) | Vitest | M1~M3 |
| Unit | `JsonRealtimeBulletinPresetRepository`, `FileBulletinBoardRepository` | Vitest + memfs or mock | M1/M3 |
| Unit | `SaveBulletinSnapshot`, `LoadBulletinSnapshot` usecase | Vitest + mock port | M2 |
| Integration | IPC handler `electron/ipc/realtimeBulletin.ts` M2/M3 채널 | Vitest + electron mock | M2/M3 |
| Integration | `ToolRealtimeBulletin.tsx` viewMode 전환 + autosave 훅 | Vitest + RTL + fake timers | M2 |
...
- [ ] 삭제 시 스냅샷도 함께 unlink
- [ ] ToolResult `boardId?` 없는 결과 로드 — `RealtimeBulletinDetail`가 crash 없이 렌더
...
| IPC 채널 | `realtime-bulletin:kebab-case` | `realtime-bulletin:save-snapshot` |
| 파일(component) | PascalCase.tsx | `PresetManagerModal.tsx` |
| 파일(utility) | camelCase.ts | `realtimeBulletinRules.ts` |
| 폴더 | kebab-case or PascalCase | `RealtimeBulletin/` (기존 유지) |
| Storage 파일 | kebab-case.json | `bulletin-boards.json`, `tool-presets.json` |
| 타입 | PascalCase | `BulletinBoard`, `RealtimeBulletinSnapshot` |
...
// 3. Usecases (@usecases/*)
import { ManageBulletinBoard } from '@usecases/realtimeBulletin/ManageBulletinBoard';

// 4. Adapters (@adapters/*)
import { useRealtimeBulletinBoardStore } from '@adapters/stores/useRealtimeBulletinBoardStore';

// 5. Relative
import { PresetPicker } from './PresetPicker';

// 6. Type-only
import type { RealtimeBulletinLayoutMode } from '@domain/entities/RealtimeBulletin';
```
...
| Item | Convention |
|---|---|
| viewMode enum | `ToolRealtimeBulletin` 내부에서만 사용. 다른 컴포넌트에 export 금지 |
| IPC handler return | 성공 시 payload 객체 반환. 실패 시 `throw new Error('CODE: message')` |
| autosave 훅 | `useRealtimeBulletinSessionStore`의 `scheduleAutosave()` 함수로 캡슐화. 컴포넌트에서 `setInterval` 직접 X |
...
2. [ ] `domain/entities/RealtimeBulletinPreset.ts` 신규
3. [ ] `domain/repositories/IRealtimeBulletinPresetRepository.ts` 신규
4. [ ] `domain/rules/realtimeBulletinRules.ts`에 `buildPresetFromConfig` 추가 + 테스트
5. [ ] `usecases/realtimeBulletin/ManagePresets.ts` 신규 + 테스트 (mock repository)
6. [ ] `adapters/repositories/JsonRealtimeBulletinPresetRepository.ts` 신규
7. [ ] `adapters/stores/useRealtimeBulletinPresetStore.ts` 신규
8. [ ] `adapters/di/container.ts` 추가 조립
9. [ ] `PresetPicker.tsx` + `PresetManagerModal.tsx` 신규
10. [ ] `ToolRealtimeBulletin.tsx` `CreateView`에 PresetPicker 통합
11. [ ] 수동 테스트: 저장/선택/이름변경/삭제/v1 포맷 로드
12. [ ] `npx tsc --noEmit` + `npx vitest run` 그린 확인

### 13.2 Phase M2 구현 순서 (3~5일)

1. [ ] `domain/entities/RealtimeBulletinSnapshot.ts` 신규
2. [ ] `domain/ports/IRealtimeBulletinSnapshotPort.ts` 신규
3. [ ] `usecases/realtimeBulletin/SaveBulletinSnapshot.ts` + `LoadBulletinSnapshot.ts` + `List/DeleteBulletinSnapshot.ts` 신규
4. [ ] `infrastructure/realtimeBulletin/FileBulletinSnapshotPersistence.ts` 신규 (fs + path.resolve 검증)
5. [ ] `electron/ipc/realtimeBulletin.ts`에 4채널 핸들러 추가 + `endActiveBulletinSessionSync` export
6. [ ] `electron/main.ts` before-quit 훅 추가
7. [ ] `electron/preload.ts` 4개 브릿지 함수 추가
8. [ ] `src/global.d.ts` `ElectronAPI` 확장
9. [ ] `adapters/stores/useRealtimeBulletinSessionStore.ts` 신규 (autosave schedule)
10. [ ] `ResumeSessionBanner.tsx` + `EndSessionDialog.tsx` 신규
11. [ ] `ToolRealtimeBulletin.tsx` autosave 훅 통합 + 종료 분기 다이얼로그
...
5. [ ] `usecases/realtimeBulletin/ManageBulletinBoard.ts` + `StartBulletinBoardSession.ts` 신규 (M2 snapshot 결합)
6. [ ] `infrastructure/realtimeBulletin/FileBulletinBoardPersistence.ts` 신규
7. [ ] `adapters/repositories/FileBulletinBoardRepository.ts` 신규
8. [ ] `electron/ipc/realtimeBulletin.ts`에 8채널 추가
9. [ ] `electron/preload.ts` `bulletinBoards` 서브객체 추가
10. [ ] `src/global.d.ts` 확장
11. [ ] `adapters/stores/useRealtimeBulletinBoardStore.ts` 신규
12. [ ] `BulletinBoardList.tsx` 신규 + 온보딩 Toast
13. [ ] `ToolRealtimeBulletin.tsx` viewMode='list' 추가, 라우팅 조정
...
- ❌ `useToolPresetStore`에 realtime-bulletin 로직을 억지로 끼워 넣어 union 타입이 사용처마다 narrowing 지옥
