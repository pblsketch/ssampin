...
| adapters | [src/adapters/di/container.ts](../../../src/adapters/di/container.ts) | **수정** — BoardRepository 조립 |
...
| electron | [electron/ipc/tunnel.ts](../../../electron/ipc/tunnel.ts) | (변경 없음) — coordinator가 driver로 주입받음 |
| electron | [electron/preload.ts](../../../electron/preload.ts) | **수정** — `window.api.board.*` 네임스페이스 |
| electron | [electron/main.ts](../../../electron/main.ts) | **수정** — `registerBoardHandlers(mainWindow)` 1줄 등록 |
...
│                                                                  │
│                ↕ window.api.board.* (preload contextBridge)      │
└──────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────┐
│ Electron Main (Node)                                             │
│                                                                  │
...
export function verifyJoinCredentials(
  ...
  if (!isAuthToken(providedToken)) return false;
  if (!isSessionCode(providedCode)) return false;
  // 타이밍 공격 방지: 전체 비교 후 결과 반환 (early return 금지)
...
절차:
  ...
  2. 포트 선점: findFreePort() (동적 40000~49999 범위)
  3. IBoardRepository.loadSnapshot(boardId) → initialState
  4. IBoardServerPort.start({ boardId, initialState, onStateChange, onParticipantsChange })
     → BoardServerHandle { localPort, authToken, sessionCode, ... }
  5. IBoardTunnelPort.acquire('board', localPort) → publicUrl
  6. qrcode.toDataURL(`${publicUrl}?t=${authToken}&code=${sessionCode}`) → qrDataUrl
  7. 30초 주기 자동 저장 타이머 시작 (setInterval)
     → encodeState() → IBoardRepository.saveSnapshot()
     → 단, onStateChange 이후 변경이 있을 때만 저장 (dirty flag)
  8. BoardSession 레코드를 useBoardSessionStore에 반영 (IPC 이벤트로 renderer에 push)
실패:
  - 4 실패 시 (서버 기동) → 리소스 없음, 그대로 throw
  - 5 실패 시 (터널 획득) → IBoardServerPort.stop(boardId) 후 throw
  - 6 실패 시 → 5·4 역순 롤백 후 throw
```
...
### 4.1 Channel 목록 (`window.api.board.*`)

| 채널명 | 방향 | 페이로드 | 응답 |
|-------|------|---------|------|
| `board.list` | R→M | `{}` | `Board[]` |
| `board.create` | R→M | `{ name?: string }` | `Board` |
| `board.rename` | R→M | `{ id: BoardId, name: string }` | `Board` |
| `board.delete` | R→M | `{ id: BoardId }` | `{ ok: true }` |
| `board.startSession` | R→M | `{ id: BoardId }` | `BoardSessionStartResult` |
| `board.endSession` | R→M | `{ id: BoardId, forceSave: boolean }` | `{ ok: true }` |
| `board.getActiveSession` | R→M | `{}` | `BoardSession \| null` |
| `board.saveSnapshot` | R→M | `{ id: BoardId }` | `{ savedAt: number }` |
| `board:onParticipantChange` | M→R | `{ boardId, names: string[] }` | (event) |
| `board:onAutoSave` | M→R | `{ boardId, savedAt }` | (event) |
| `board:onSessionError` | M→R | `{ boardId, error: string }` | (event) |
...
| `BOARD_PORT_EXHAUSTED` | "사용 가능한 포트를 찾을 수 없습니다" | findFreePort 100회 재시도 실패 |
...
export function registerBoardHandlers(mainWindow: BrowserWindow): void {
  const repo = container.boardRepository();
  const serverPort = container.boardServerPort();
  const tunnelPort = container.boardTunnelPort();
  const useCases = {
    manage: new ManageBoard(repo),
    start: new StartBoardSession(repo, serverPort, tunnelPort),
    end: new EndBoardSession(repo, serverPort, tunnelPort),
    saveSnapshot: new SaveBoardSnapshot(repo, serverPort),
  };

  ipcMain.handle('board.list', async () => useCases.manage.listAll());
  ipcMain.handle('board.create', async (_e, { name }) => useCases.manage.create(name));
  // ... (동일 패턴)

  serverPort.on('participantChange', ({ boardId, names }) =>
    mainWindow.webContents.send('board:onParticipantChange', { boardId, names }),
  );
  serverPort.on('autoSave', (ev) =>
    mainWindow.webContents.send('board:onAutoSave', ev),
  );
}
```

기존 [electron/ipc/liveMultiSurvey.ts](../../../electron/ipc/liveMultiSurvey.ts) 패턴과 **완전히 동일한 시그니처**.
...
정적 부분(HTML/CSS/JS 본문 ~15KB)은 `generateBoardHTML(boardName, token, code)`가 템플릿 문자열로 생성. Excalidraw·React·Y.js는 전부 CDN.
...
| **학생 HTML** | 인증 실패 (1008) | Full-screen overlay: "연결할 수 없습니다. 선생님께 QR 코드를 다시 받아주세요." |
| **학생 HTML** | WebSocket close unexpected | 상태 뱃지 "연결 끊김" + Exponential backoff 재연결 (1s, 2s, 4s, ..., max 30s) |
| **학생 HTML** | Excalidraw CDN 로드 실패 | Fallback: `jsdelivr` CDN으로 재시도 (`<script>` 태그 2개 순차 시도) |
...
- [ ] **이름 공백**: 공백/제로폭만 입력 → 거부
- [ ] **최대 인원**: 50명 초과 시 1013 close
- [ ] **Excalidraw CDN fallback**: unpkg 블록 시 jsdelivr 경유 로드 확인 (수동)
...
| IPC 채널 | `board.` (요청) / `board:` (이벤트) | `board.startSession`, `board:onParticipantChange` |
...
### 11.2 Implementation Order (권장 순서)

**Phase 1a MVP 구현 순서 (의존성 역순)**:

1. **domain/** 전체 (값 객체 → 엔티티 → 규칙 → 포트/레포 인터페이스) — 약 0.5일
2. **infrastructure/board/** — 약 1주
   - `constants.ts`
   - `BoardTunnelCoordinator.ts` (스파이크 s2 코드 이식)
   - `BoardFilePersistence.ts`
   - `YDocBoardServer.ts` (y-websocket setupWSConnection + 인증 필터)
   - `generateBoardHTML.ts` (스파이크 board.html 템플릿화)
3. **usecases/board/** — 약 2~3일
   - `ManageBoard`, `StartBoardSession`, `EndBoardSession`, `SaveBoardSnapshot`, `AuthorizeBoardJoin`
4. **adapters/repositories/FileBoardRepository.ts** + **di/container.ts 수정** — 약 1일
5. **electron/ipc/board.ts** + **preload.ts/main.ts 수정** — 약 0.5일
6. **adapters/stores/** — 약 0.5일
7. **adapters/components/Tools/Board/** + **ToolCollabBoard.tsx** — 약 3~4일
8. 수동 통합 테스트 (교사 PC + 실기기 5~10대) — 약 2일
9. AI 챗봇 KB 업데이트 + 릴리즈 노트 — 약 0.5일

**합계 예상**: 2~3주 (Plan 추정 범위 내)
