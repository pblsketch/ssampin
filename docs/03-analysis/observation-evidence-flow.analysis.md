...
### 8. 검토 후 수정 (2026-09-07, 커밋 `134f3e2e`·`71d31c1a`·`1d92ca9c`·이번)

실기기 QA 전에 S0~S4 전체를 네 갈래로 검토했다(저장·잠금 / 학생 경계·왕복 / 비교·상태표현 /
입력화면·테스트품질). **게이트 4종이 전부 초록인 채 존재한 결함 13건**이 나왔고 전부 고쳤다.

★검토가 확인한 좋은 소식부터: **저장·잠금 층은 계약대로였다.** 잠금 순서 역전 0, 메모리 유령 0,
읽기 실패 전파, 동시 요청 중복 방지, 되돌리기 쓰기 0회가 실제로 지켜지고 그 테스트는 실물 락을
쓴다. 결함은 전부 **그 위층 배선**에 몰려 있었다.

#### QA 를 막던 것

| 결함                                    | 무엇이 잘못됐나                                                                                                                                                                                                                           |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [근거 보드에서 보기]가 보드를 안 열었다 | 인텐트 소비부가 학생만 고르고 `setViewMode('evidence')` 를 안 불렀다. 바로 위 `openBoardFor` 에 올바른 코드가 있는데 안 썼다. `sourceId`·`evidenceId`·`threadId` 는 한 번도 읽히지 않았고 `needsFilterReset` 은 프로덕션 호출자가 0이었다 |
| 담임 이어 쓰기가 출결 그리드로 갔다     | 상위 탭만 바꾸고 **하위 탭**(별도 스토어, 기본값 '출결')을 안 건드렸다. 요청은 소비되지 않고 남아 있다가, 나중에 '누가기록'을 누르는 순간 되살아나 학생 선택을 갈아쳤다                                                                   |
| 비교창 재확인이 영원히 실패했다         | 자식 effect 가 부모보다 먼저 flush 돼 **옛 원본으로 다시 캡처**하고, 그 뒤 `capture !== null` 가드에 막혀 고정됐다. 창을 닫았다 여는 수밖에 없는데 화면은 그 길을 안 알려 줬다                                                            |

★세 번째는 임시 테스트로 **실측**한 뒤 고쳤다(추론으로 끝내지 않았다). 화면은 최신 원본을
보여 주는데 `capture.source.content` 는 옛 값이었다.

#### 데이터가 조용히 틀어지던 것

- **교과 실패 첨부가 다음 관찰에 붙었다.** 실패분을 대기 목록에 남기면서 기록 id 를 안 챙겨,
  다음 저장이 `commitPendingAttachments(새 recordId, 앞 기록의 파일)` 을 불렀다. 앞 관찰의
  학생 제출물이 다른 관찰의 증빙이 되는 사고다. 자동저장이 이미 쓰던 방식(맥락과 함께 보관 +
  명시적 재시도)으로 맞췄다.
- **담임에서 남의 학생에게 주제가 생겼다.** 학생을 바꿀 때 주제를 지우는 코드가 없었다
  (교과는 있다). `kind:'new'` 는 스토어 소유권 검사에 **걸릴 대상 자체가 없어** 막히지도 않는다.
  주제에 주인(`studentRef`)을 묶어 다르면 없는 것으로 본다.
- **교과 학생 전환 자동저장이 주제를 버렸다.** 첨부는 캡처하는데 주제만 빠져 있었다(AC-05).
- **담임 첨부 일부 실패 후 재시도할 파일이 사라졌다.** `commitPendingAttachments` 의 반환값을
  버려 `resetForm` 이 방금 보존한 실패분을 지웠다. 토스트는 "다시 시도해 주세요"라고 했다.
- **담임 원본 수정에서 장면 전부 해제가 저장 안 됐다.** `{...record}` 가 옛 `slots` 를 싣고 있어
  조건부 spread 가 안 덮었다. 교과는 `const { slots: _prev, ...rest }` 로 정확히 피하고 있었다.

#### 실패가 조용하던 것

- 교과 [기록 저장] 실패에 `catch` 가 없어 unhandled rejection 으로 끝났다. 같은 파일 자동저장
  경로에는 이미 있던 안내가 명시 저장에만 없었다.
- Ctrl+Enter / Enter 연타로 같은 기록이 두 벌 저장됐다. 단추는 state 로 막히지만 키보드는 통과했고,
  **원본이 둘이면 `sourceId` 가 달라 스토어의 중복 방지가 발동하지 않는다.** ref 로 막았다.
- 최근 기록이 "근거 목록을 아직 안 읽음"을 **'주제 미지정'이라 단정**했다. 근거 스토어를 읽는 곳이
  초안·보드 화면뿐이라, 앱을 켜고 바로 수업 기록에 오면 묶인 기록까지 전부 미지정으로 보였다.
  `RecordTopicStatus` 에 `pending` 을 두고 '주제 확인 중'이라고 말한다(ADR-086 결정 2).

#### 테스트가 잠근 게 없던 자리

★**`observationEvidenceSave.ts`(175줄)를 부르는 프로덕션 코드가 0개였다.** 그 테스트 11건이
AC-06·09 의 체크포인트 계약을 잠근 것처럼 보였지만 **화면에는 그 계약이 존재하지 않았다.**

두 화면이 쓰기에는 모양이 안 맞았다(담임 저장은 다학생·다날짜라 단일 레코드 흐름에 안 들어간다).
그래서 **함수와 테스트를 지우고**, 그 계약 중 실제로 빠져 있던 하나(**연결만 실패했을 때
'연결 다시 시도'**)를 두 화면에 직접 넣었다. 나머지(재시도가 원본을 새로 안 만듦, 실패한 파일만
다시 올림, 남의 학생 체크포인트 안 이어받음)는 위 수정으로 이미 화면에 들어왔다.

★담임 재시도는 값을 **인자로 받게** 바꿨다. 폼 상태를 클로저로 읽으면 `resetForm` 이 돈 뒤 눌리는
재시도가 빈 본문·빈 주제로 실행된다.

다른 두 개도 고쳤다:

- `useRecordSaveStatus.test.ts` 가 `wrapSave` 를 8번 부르면서 **반환값을 한 번도 단언하지 않았다.**
  이번 작업의 핵심이 "성공 여부를 돌려주게 바꿨다"인데 `return true` 로 고정해도 전부 통과했다.
- `evidenceSourceComparison.test.ts` 의 「줄바꿈 표기만 달라진 것은 바뀜이 아니다」가 **CR 을 아예
  넣지 않아** 위 케이스의 복사본이었다. 정규화를 통째로 지워도 통과했다. CRLF vs LF 로 다시 쓰고,
  정규화를 제거하면 실제로 실패하는 것을 확인했다.

#### 새로 만든 테스트

- `Homeroom/Records/__tests__/inputModeSaveContracts.test.tsx` — **`InputMode` 첫 렌더 테스트.**
  이 화면에는 그동안 렌더 테스트가 하나도 없었고(참조 4건은 전부 소스 문자열 grep) 위 결함 다수가
  그 구멍으로 나갔다. 소유권 검사를 빼면 **실제로 실패하는 것**을 확인했다.
- 비교창: 재검증 실패 뒤 재확인이 통하는지 · 보드가 대상을 찾아 포커스하는지 · 못 찾으면 말하는지.
- `recordTopicLabel`: 안 읽음과 미지정을 구별하는지.

★새 테스트는 전부 **고치기 전 코드에서 실패하는지 확인**하고 넣었다. 통과만 하는 테스트는
잠근 것이 없다는 것을 이번에 크게 겪었다.
