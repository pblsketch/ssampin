...
## S4 — 원본 비교·명시 반영 (2026-09-07, 완료)

저장 계약 `b3f3a764` + 비교창 화면(이번 세션).
...
### 2. 원본을 스토어가 아니라 저장소에서 읽는다

`useEvidenceSourceState`(신규, adapter)가 `observationRepository`·`studentRecordsRepository` 를 직접 읽는다.
스토어를 쓰지 않은 이유가 하나뿐인데 결정적이다.

> `useObservationStore.load` 와 `useStudentRecordsStore.load` 는 **읽기 실패를 catch 로 삼키고**
> `loaded: true` 로 끝낸다. 그래서 스토어만 보면 "파일을 못 읽었다"와 "원본이 없다"가
> 똑같이 **빈 목록**으로 보인다.

그 상태로 화면을 그리면 멀쩡한 원본을 '원본을 찾을 수 없습니다'라고 말한다. 계획 §5.3 이
명시적으로 금지한 오진이고, AC-15 가 그걸 잡으라고 있는 항목이다. 그래서 한 번 더 읽고
실패는 실패로 남긴다(`status: 'loading' | 'error' | 'ready'`).

| 상태           | 화면                                       | 왜                                         |
| -------------- | ------------------------------------------ | ------------------------------------------ |
| `loading`      | '원본 확인 중'                             | 모르는 동안에는 판정하지 않는다            |
| `error`        | '원본을 불러오지 못했습니다' + [다시 시도] | **삭제됐다고 말하지 않는다**               |
| `missing`      | '원본을 찾을 수 없습니다'                  | 읽기가 성공한 뒤에만 이렇게 말할 수 있다   |
| `found`        | 값 + 차이 표시                             | 여기서만 '다름/같음'을 말한다              |
| `out-of-scope` | 상태 줄 없음                               | 평가·과제물·첨부는 비교 범위 밖(계획 §5.3) |

★`readLatestSource` 는 **부를 때마다 다시 읽는다.** 화면이 들고 있는 값으로 대조하면
대화상자를 열어 둔 사이의 변경을 못 잡아 재검증 자체가 무의미해진다.
★읽기 실패는 **던진다.** `null`(정말 없음)로 뭉개면 스토어가 '삭제됨'으로 처리한다.
★그사이 다른 학생 것이 됐으면 `null` 이다. 소유권이 바뀐 원본으로 근거를 덮으면 남의 기록이 섞인다.

### 3. 반영은 2단계다 (디자인 협업 결과)

`EvidenceSourceComparisonDialog` 는 공용 `Modal`(focus-trap-react) 위에 두 칸을 얹는다.
디자이너와 협의해 **버튼 한 번이 아니라 두 번**으로 바꿨다.

1. [원본 내용으로 바꾸기] → 미리보기만 펼친다. **쓰기 0회.** 다른 칸만 `지금 → 바뀔 값` 으로 보여 준다.
2. 미리보기 안의 [이 내용으로 바꾸기] → 여기서 비로소 `applySourceFields`.

★기본은 [현재 근거 유지]다. 덮어쓰기가 기본이면 습관적인 확인 한 번에 다듬은 글이 날아간다.
★데이터가 아예 없는 상태(확인 중·실패·없음)에서는 [바꾸기]를 **비활성이 아니라 숨긴다.**
누를 수 없는 단추가 남아 있으면 "곧 눌릴 것"이라고 읽힌다. 조건만 안 맞는 상태(빈 본문·출결)
에서만 비활성으로 두고 **왜 잠겼는지**를 붙인다(비활성 단추의 `title` 은 스크린리더가 안 읽어
`sr-only` 로 한 번 더 적었다).
★재검증에 걸리면 창을 닫지 않는다. 미리보기를 걷고 비교를 새로 잡아 다시 확인받는다.

### 4. 카드는 '다름'만, 상세는 다 말한다

- 카드: **'원본과 내용이 달라요' + [비교하기]만.** 목록에 '같음'이 줄줄이 붙으면 정작 봐야 할
  '다름'이 묻힌다. 확인 중·확인 실패·원본 없음은 전부 표시 없음이다 — **모르는 것을 다르다고
  말하지 않는다.**
- 상세('근거 수정' 폼): 6갈래를 다 말한다(확인 중·실패·없음·빈 본문/출결·같음·다름).
  한 건만 보는 자리라 시끄럽지 않다.
- 거울 카드에는 안 붙는다. 원본 그 자체라 비교 대상이 아니다.

### 5. 삭제 — 확인한 것만 약속한다

라벨을 `sourceId` 있으면 '정리한 근거 삭제'로 바꾸고, 안내를 4갈래로 나눴다
(`evidenceDeleteGuidance`, 도메인 순수 함수).

| 갈래             | 조건                                        | 문구                                                              |
| ---------------- | ------------------------------------------- | ----------------------------------------------------------------- |
| `reappears`      | 원본 확인 성공 + 거울 적격                  | '정리한 근거를 지웠습니다 · 원본은 미분류에 다시 표시됩니다'      |
| `source-missing` | 읽기 성공했는데 없음                        | '정리한 근거만 지웠습니다'                                        |
| `evidence-only`  | 읽기 실패 · 출결 · 공백 본문 · 범위 밖 출처 | '정리한 근거만 지웠습니다 · 원본은 이 동작으로 지우지 않았습니다' |
| `manual`         | 직접 입력(출처 없음)                        | '근거 1건을 지웠습니다'(기존 그대로)                              |

★**조회 실패는 "모르는 것"이다.** 확인하지 못한 채 "미분류에 다시 표시됩니다"라고 하면
교사는 "지웠다가 다시 담으면 되지"로 판단한다. 확인된 경우에만 약속한다.
★문구 판정은 **지우기 전에** 한다. 지운 뒤에는 거울 후보가 되살아나 상태가 달라진다.
★확인 대화상자는 새로 만들지 않았다(디자인 결정). 원본을 절대 지우지 않고 5초 되돌리기가
이미 있어 사전 차단이 필요한 경고가 아니다. 보드의 다른 삭제와 패턴을 통일했다.

### 6. 되돌리기는 `add` 가 아니다 (AC-16 (c))

기존 되돌리기는 `addEvidence({...})` 로 새 근거를 조립했다. 두 군데가 틀렸다.

1. `add` 는 **AI 제외를 다시 판정한다.** 금지 어휘가 든 본문에서 교사가 일부러 꺼 둔 제외가
   되살아난다. 되돌리기는 판정이 아니라 **복원**이다.
2. `add` 는 같은 원본이 이미 있으면 **그 id 만 돌려주고 끝난다.** 화면은 '되돌렸습니다'라고
   말하는데 실제로는 아무 일도 안 일어났고, 그사이 교사가 새로 다듬은 글이 있으면 그것도 못 알린다.

그래서 스토어에 `restoreRemoved(evidence)` 를 새로 뒀다. 원래 레코드를 **통째로**(id·createdAt·
주제·영역·제외까지) 다시 넣고, 자리가 이미 찼으면 **쓰기 0회**로 `{ restored: false }` 를 돌려준다.
화면은 그때 '이미 새로 저장된 근거가 있어 되돌리지 않았습니다 · 그 근거를 확인해 주세요'라고 말한다.
...
| -------------------------- | --------------------------------------------- |
| `npx tsc --noEmit`         | 에러 0                                        |
| `npm run lint`             | 0 error · 137 warning (기준선과 동일, 신규 0) |
| `npm run test`             | 698 파일 · 9,338 통과 · 10 skip · 실패 0      |
| `npm run regression-check` | 55/55 통과                                    |

새로 잠근 테스트:

- `domain/rules/__tests__/evidenceSourceComparison.test.ts` +7 — 삭제 안내 4갈래(AC-16 (b)).
- `adapters/hooks/__tests__/useEvidenceSourceState.test.ts` 14건 — 읽기 실패 vs 없음, 범위 밖,
  `readLatestSource` 의 재읽기·던지기·소유권(AC-15).
- `adapters/stores/__tests__/recordEvidenceSourceLifecycle.test.ts` +4 — `restoreRemoved` 의
  원형 복원·자리 참(쓰기 0회)·두 번 누르기(AC-16 (c)).
- `adapters/components/RecordDraft/__tests__/evidenceSourceComparison.board.test.tsx` 16건 —
  AC-13·14·15·16 화면 경로.

★기존 보드 테스트의 되돌리기 단언을 `add` 에서 `restoreRemoved` 로 바꿨다. 계약이 바뀌었으니
테스트도 바뀌는 것이 맞다 — 옛 단언을 남겨 두면 위 1·2번 결함을 테스트가 지켜 준다.

### 8. 남은 것

S5(실기기 QA · 공개 가이드 `landing/src/content/docs.ts` 갱신). 특히 AC-18 은 실제 렌더가 필요하다:
1280/1920 · 큰 글꼴 · 밝음/어두움/**유리** 모드에서 두 칸이 잘리지 않는지, 미리보기 펼침·걷기의
포커스 이동이 실기기에서도 맞는지는 자동 테스트로 못 본다.
