...
| T2-2 `RecordDraft.threadId?`          | ⚠️ **절반만**      | 엔티티·스토어 입력·브릿지 미러까지는 됐지만 **값을 넣는 쓰기 경로가 없다**(Codex QA 발견). 초안 화면 `persist()` 와 브릿지 write 계약 둘 다 threadId 를 안 싣는다 → "이 주제로 쓴 초안" 조회는 **아직 성립하지 않는다.** 배선은 UX 결정(주제 고르기가 보기 필터인가 소속 지정인가)이 필요해 남긴다. |
