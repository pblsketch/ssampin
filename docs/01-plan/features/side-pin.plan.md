...
- 상태: **정식 기획서 확정 · 구현 진행 중** (§9-1·9-2·9-4 완료, §9-3에서 대기)
- 확정일: 2026-08-13
- 계보: v0.2 초안(`side-pin-v0.plan.md`, 보관) → v0.3 합의안 → **v0.4 코드 대조 QA 반영본(이 문서)**
- 이 문서가 유일한 구현 기준이다. QA에서 바뀐 항목은 §20, 구현 현황과 정정은 §21에 있다.
- **다음 단계는 §21 "다음 세션이 여기서부터 시작한다"를 먼저 읽을 것.** §9-3은 실기기 실측이 필요해 AI 단독으로 통과 판정할 수 없다.
...
### §9-4 완료 — 창 위치 계산과 기기 설정 저장

§9-3(성능 spike)보다 먼저 했다. 실기기 없이 완결되고, 이 기능의 가장 큰 위험(다중 모니터·배율에서 창이 화면 밖으로 밀림)을 여기서 막기 때문이다.

| 파일                             | 역할                                            |
| -------------------------------- | ----------------------------------------------- |
| `electron/sidePinGeometry.ts`    | 손잡이·패널 위치 계산 (순수, Electron import 0) |
| `electron/sidePinDeviceState.ts` | 정규화 MIRROR 블록 + 원자적 파일 저장·복구      |
| `+ .test.ts` 3개                 | 83개 (기하 16 · 미러 53 · 저장 14)              |

**구조 제약을 확인했다.** `tsconfig.electron.json`의 `rootDir: "electron"` 때문에 **electron 프로덕션 코드는 `src/`를 import할 수 없다.** 그래서 도메인 순수 함수를 의도적으로 복제하고 미러 테스트로 동기화를 강제한다(`archiveRules ↔ archiveManager` 선례). 테스트 파일은 빌드 대상이 아니라 `../src/`를 import해도 된다. 앞으로 §9-5 이후 electron 코드가 도메인 규칙을 써야 할 때마다 같은 방식을 따른다.
...
### §9-6 일부 — 손잡이와 패널 화면 (frontend-design 협업)

| 파일                                               | 역할                                |
| -------------------------------------------------- | ----------------------------------- |
| `src/adapters/components/SidePin/SidePinRail.tsx`  | 손잡이 — 두 구역(위젯·메모) 입구    |
| `src/adapters/components/SidePin/SidePinPanel.tsx` | 패널 껍데기 — 헤더 + 위젯/메모 영역 |
| `+ .test.tsx` 2개                                  | 24개 (동작 13 + **디자인 규칙 11**) |

디자인은 사용자가 확정한 규칙(`sp-*` 토큰, Noto Sans KR, 안티레퍼런스 "과도한 네온·글로우")을 우선하고, 협업 스킬의 지침 중 그와 충돌하는 부분(개성적 폰트·과감한 독자 미감)은 적용하지 않았다.

**⚠️ 기획서 §5.1을 바꾸는 발견 — 손잡이는 화면 높이 전체가 아니다.**

§5.1은 손잡이 폭만 정하고 높이는 정하지 않았고, 초기 구현은 작업 영역 전체 높이로 계산했다. 그런데 손잡이 창은 **항상 위에 떠 있으므로**, 가장자리 전체를 덮으면 그 줄이 통째로 클릭을 가로채 **최대화한 창의 스크롤바를 누를 수 없게 된다.** v2.3.8 위젯 손잡이 사고("겹쳐 보이는 것보다 안 눌리는 게 컸다")와 같은 계열이다.

→ **세로 가운데에 168 DIP 짧은 탭**으로 바꿨다(`SIDE_PIN_RAIL_HEIGHT`). 가장자리가 자유로워지고, 브랜드('핀')와도 맞는 교무수첩 책갈피 형태가 된다. "손잡이가 화면 가장자리를 위아래로 막지 않는다"를 테스트로 고정했다.

**그 밖의 디자인 결정**

- **화면 쪽만 둥글다**(`rounded-l-lg` / `rounded-l-xl`). 오른쪽은 화면 끝에 딱 붙어, 떠 있는 막대가 아니라 가장자리에 꽂힌 물건으로 읽힌다. 손잡이와 패널이 같은 규칙이라 접혔다 펴져도 같은 물건이 커진 것처럼 보인다.
- **상태 변화는 밝기가 아니라 얇은 강조선으로.** 글로우는 이 제품의 안티레퍼런스다.
- **위젯과 메모를 탭으로 갈아 끼우지 않는다.** 위아래로 나란히 둔다 — 하나를 보려고 다른 하나를 덮으면 "잠깐 확인하고 닫는다"는 목적이 무너진다. 이 결정을 테스트로 고정했다.
- **등장 모션은 6px·160ms 한 번.** 펼침 예산이 300ms인데 연출이 그걸 먹으면 사용자에게는 느린 앱이 된다. 기존 `animate-sp-fade-in`의 토큰·절제 수준을 그대로 따랐고, `prefers-reduced-motion`에서는 움직이지 않는다.

**디자인 규칙을 테스트로 고정했다.** 하드코딩 색·글로우·직각·`rounded-sp-*`·`sp-*` 투명도 수식·raw `text-white`·스크롤 영역의 `min-h-0` 누락을 소스 검사로 막는다. 전부 이 저장소에서 반복해 지적된 것들이라, 사람이 매번 눈으로 보는 대신 그물을 뒀다. 일부러 `bg-sp-accent/50 drop-shadow`를 넣어 **정확히 두 개만 빨간불이 되는 것**을 확인했다.

**남은 §9-6**: 렌더러 상태 연결(§9-5 IPC), `src/App.tsx`의 `mode=sidePin` 분기, 위젯·메모 내용물(§9-7·9-8), 라이트/다크 실렌더 확인.
