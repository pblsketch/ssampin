...
M1~M3 + 후속 2건까지 전부 끝났다. 커밋 `57fe2530`.

| 항목                  | 결과                                          |
| --------------------- | --------------------------------------------- |
| M1 순수 규칙 + 테스트 | `consultationTimetableRules.ts` · 테스트 45건 |
| M2 날짜별 계산 배선   | 제외 키를 `날짜\|교시` 로                     |
| M3 화면               | 날짜·요일 표시 + 접기 + 요약 줄               |
| 후속① 편집 화면       | 시간대 줄마다 [수업 빼기]                     |
| 후속② 만든 뒤 안전망  | 겹침 **안내만** — 자동 차단 안 함             |
| 기록                  | ADR-076                                       |

**게이트**: tsc 0 · lint 0 error(경고 135, 변경 전과 동일) · vitest 8,252 통과 ·
regression 51/51. **실화면**: 요일이 다른 두 날짜·토요일·날짜 4개 밀도·안내 배너·
[한 번에 막기]·[수업 빼기]·저장까지 확인. 검증용 상담 일정은 보관 후 삭제했다.

### 🔴 릴리즈 때 반드시 할 것 — /docs 가이드가 **되돌려진 채로** 있다

사용자 가이드 갱신분(`19bb5cdb`)을 **되돌려 두었다**(`60fa62f9`). 내용이 틀려서가 아니라
**아직 출시되지 않았기 때문이다** — 랜딩은 main 푸시 시 자동 배포되는데, 가이드는
"v2.4.5 기준" 이라고 적고 있어 지금 나가면 없는 버튼을 안내하게 된다. 이 저장소는 여러
세션이 main 을 공유해서 "푸시를 미룬다"로는 막을 수 없어 되돌리는 쪽을 택했다.

```bash
git revert 60fa62f9        # 되돌림을 되돌린다 (= 가이드 내용 복구)
# 또는
git cherry-pick 19bb5cdb   # 원본을 다시 적용해도 결과는 같다
```

되살린 뒤 릴리즈 SOP 4단계에서 함께:

- `landing/src/content/docs.ts` 의 **"지원 기준 버전"** 문단을 새 버전으로
- **"최근 반영된 주요 기능"** 에 항목 추가
- `cd landing && npm run docs:check && npm run build`

### 남겨 둔 것 (이번 범위 밖)

- **학생 상담 흐름**은 손대지 않았다. 원래 날짜별로 맞게 동작한다.
- **선생님이 상담 상세 화면을 열지 않으면** 겹침 안내를 못 본다. 자동 차단을 안 하기로
  한 대가이고, 의도한 교환이다(ADR-076 결정 6).
