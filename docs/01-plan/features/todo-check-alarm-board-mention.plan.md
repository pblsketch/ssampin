...
**0. ✅ 결정됨 (2026-08-22 오너 확정) → ㉮ 그대로 둔다. 코드 변경 없음.**

멘션으로 고른 이름을 할 일 제목에 그대로 남긴다. 이름은 할일 목록·대시보드 위젯·옆핀에
보이고 구글 Tasks 로도 올라간다. 이를 **"선생님이 직접 친 글자"** 로 본다.

- 관련인 **데이터**(`relatedStaff`)는 여전히 위젯·옆핀 경로로 흐르지 않는다 — 그 방어는 유지된다.
- 프라이버시 테스트(`SidePinWidgetZone.privacy.test.tsx`)의 보장 범위 주석이 이미 이 사실을
  정확히 적고 있다. **"이름이 어디에도 안 뜬다"고 넓게 약속하지 않는다.**
- 나중에 ㉯(제목에서 지움)로 바꿔도 저장 데이터는 같다 — `applyMention` 을 삭제형으로
  바꾸기만 하면 되므로 되돌리기 비용이 낮다.

---

<details>
<summary>당시 선택지 (기록용)</summary>

**멘션으로 고른 이름을 할 일 본문에 남길까요?**
