...
author: pblsketch
project: ssampin
version_target: v1.12.x (realtime-wall 정식 승격 기점)
---

# 쌤핀 실시간 담벼락 — 관리 기능 기획안

> **요약**: 실시간 담벼락(BETA)에 **"여러 개 개설 + 저장 + 수정관리"** 3축을 전부 얹는다. 구체적으로 (A) **설정 프리셋**(제목/레이아웃/컬럼 세트 저장 후 재사용), (B) **진행 중 세션 자동 저장/재개**(앱 종료·의도치 않은 종료 복구), (C) **보드 엔티티화**(collab-board처럼 목록/생성/이름변경/삭제/세션 시작)를 하나의 기능 묶음으로 설계한다.
>
> **핵심 방향**: 기존 realtime-wall 코드(도메인/IPC/UI)를 **부수지 않고 증강**한다. "결과 복기"만 있던 저장소 위에 "관리 대상으로서의 보드(Bulletin Board)"를 새 엔티티로 얹고, collab-board가 먼저 검증한 `list/create/rename/delete/start-session/save-snapshot` 14채널 IPC 패턴을 **미러링**한다.
>
> **Project**: ssampin (쌤핀)
...
교사가 실시간 담벼락을 **"매번 처음부터 만드는 1회용 도구"** 가 아니라 **"내 수업의 상시 자산"** 으로 쓸 수 있게 한다. 구체적 수업 시나리오:
...
- **현재 상태(2026-04-21)**: realtime-wall은 BETA 배지로 막 출시. 코어(WebSocket + cloudflared 터널 + dnd-kit + react-rnd + `tool-results.json` 읽기 전용 복기)는 완성.
...
- **프리셋 레퍼런스**: [`useToolPresetStore.ts`](../../../src/adapters/stores/useToolPresetStore.ts)가 roulette/random/wordcloud의 프리셋을 `tool-presets.json` 단일 파일로 CRUD하는 패턴을 확립. 동일 구조를 realtime-wall용으로 확장.
- **사용자 직접 요청(2026-04-21)**: "게시판 여러 개 개설 + 저장 + 수정관리 3가지 모두 필요해"

### 1.3 Related Documents

- 상위 기능 계획: [docs/01-plan/features/realtime-wall.plan.md](realtime-wall.plan.md) — 이번 계획은 여기 §7 P3(저장/복기 완성)와 P4(후속 확장: 컬럼 템플릿/세션 시작 전 레이아웃 설정 UI)를 실제 설계로 전환
...
- 결과 복기 현 상태: [`src/adapters/components/Tools/TemplateManager/PastResultsView.tsx`](../../../src/adapters/components/Tools/TemplateManager/PastResultsView.tsx) (실시간 담벼락 분기 추가됨 — 보존)
- 타입 정의: [`src/domain/entities/RealtimeWall.ts`](../../../src/domain/entities/RealtimeWall.ts), [`src/domain/entities/ToolResult.ts`](../../../src/domain/entities/ToolResult.ts) (`RealtimeWallResultData` 유지)
...
- [ ] 도메인: `src/domain/entities/RealtimeWallPreset.ts` — `id/name/layoutMode/columnTitles[]/createdAt/updatedAt`
- [ ] 저장소: `tool-presets.json`의 `ToolPresetType`에 `'realtime-wall-preset'` 추가 (기존 단일 파일 유지), 또는 전용 `realtime-wall-presets.json` 분리 — **§6 Architecture Considerations에서 결정**
- [ ] Zustand: `useRealtimeWallPresetStore` (load/getAll/add/rename/update/delete)
...
- [ ] 단위 테스트: 프리셋 정규화(빈 컬럼 제거 등 `buildRealtimeWallColumns` 재사용) + 이름 유일성 검증

### 2.2 In Scope — Phase M2 (진행 중 세션 자동 저장/재개, v1.12.x 다음 마이너)

> 교사가 가장 크게 체감하는 안정성 향상. 여기부터는 IPC 신설이 필요하다.

- [ ] 도메인: `src/domain/entities/RealtimeWallSnapshot.ts` — `id(=boardId)/title/layoutMode/columns/posts[]/savedAt/sessionState('running'|'paused'|'ended')`
- [ ] 포트: `src/domain/ports/IRealtimeWallSnapshotPort.ts` — `saveSnapshot/loadSnapshot/listSnapshots/deleteSnapshot`
- [ ] 어댑터: `src/infrastructure/realtimeWall/FileBulletinSnapshotPersistence.ts` — `userData/data/bulletins/{id}.json` 경로
- [ ] IPC 신설 (M2용 4채널):
  - `realtime-wall:save-snapshot` (M→R 결과 반환) — debounced 30초 자동 저장 + 수동 저장
  - `realtime-wall:load-snapshot` — 특정 boardId 스냅샷 로드
  - `realtime-wall:list-snapshots` — 재개 가능 목록
  - `realtime-wall:delete-snapshot`
...
- [ ] 런타임: `ToolRealtimeWall.tsx` posts/columns 변경 시 debounced(3초) autosave 호출
...
- [ ] IPC 신설 (M3용, collab-board 14채널 미러):
  - `realtime-wall:list` / `:create` / `:rename` / `:delete`
  - `realtime-wall:start-session` (snapshot 로드 → 세션 활성화 연계)
  - `realtime-wall:end-session`
  - `realtime-wall:get-active-session`
  - (M2의 save/load/list/delete-snapshot은 여기서 통합 사용)
- [ ] UI 신설: "실시간 담벼락" 진입 시 지금의 CreateView 대신 **보드 목록 화면** (카드 리스트: 이름/레이아웃/마지막 수정/액션 3개)
...
- [ ] 라우팅: `tool-realtime-wall`은 **목록 페이지**, 보드 편집/실행은 서브 상태(컴포넌트 내부 view state) — 전체 새 PageId 도입은 지양 (Sidebar 오염 방지)
- [ ] 마이그레이션: 기존 ToolResult의 `realtime-wall` 타입 결과에 `boardId` 없을 수 있음 → `boardId?` optional로 두고 고아 결과는 "분류되지 않은 세션" 섹션에 별도 표시
...
- **실시간 공동 편집**(여러 교사가 동시에 한 보드 관리): collab-board가 해당 영역 담당. 실시간 담벼락은 교사 1인 주도 모델 유지.
...
| FR-04 | 진행 중인 실시간 담벼락 세션은 **30초마다 자동 스냅샷 저장**된다 | High | M2 | Pending |
...
| FR-09 | 실시간 담벼락 진입 시 **보드 목록 화면**이 먼저 표시된다 (프로젝트 내 생성된 모든 보드) | High | M3 | Pending |
...
| FR-13 | 기존 realtime-wall의 **모든 동작(학생 참여/승인/고정/숨김/보드 조작/결과 저장)** 은 1줄도 퇴행하지 않는다 | High | M1~M3 | Pending |
| FR-14 | 기존 저장된 ToolResult의 `realtime-wall` 결과는 `boardId` 없이도 계속 표시된다 | High | M3 | Pending |
...
- [ ] 기존 실시간 담벼락 BETA 사용자가 v1.12.x 업그레이드 후 **데이터 손실 0건** (프리셋 마이그레이션, ToolResult 포맷 유지)
...
- [ ] realtime-wall BETA 배지는 **M3 완료 시점에 제거** 검토 (기획자 결정)
...
|------|--------|------------|------------|
| **IPC 채널 충돌**: collab-board 14채널과 realtime-wall 14채널이 비슷한 이름으로 혼선 | Medium | Medium | 네이밍 규칙 고정: `collab-board:*` vs `realtime-wall:*` prefix로 강제 분리. 통합 라우터 도입하지 않음. |
| **before-quit 동기 저장 교착**: collab-board가 이미 `endActiveBoardSessionSync` 사용 중. realtime-wall도 동일 훅 추가 시 순서/타임아웃 이슈 | High | Low | `main.ts`에서 **순차 호출**(collab 먼저, bulletin 다음). 각 핸들러 타임아웃 300ms cap. 실패해도 quit는 진행. |
...
|----------|---------|----------|-----------|
| **프리셋 저장 위치** | (a) 기존 `tool-presets.json` 확장 / (b) 전용 `realtime-wall-presets.json` | **(a)** | 단일 진실원(single source) + useToolPresetStore 확장 최소. ToolPresetType에 `'realtime-wall-preset'` 추가. |
...
| **보드 진입 UI 패턴** | 별도 Sidebar 항목 / 기존 `tool-realtime-wall` 내부 뷰 분기 | **내부 뷰 분기** | Sidebar 오염 방지. `ToolRealtimeWall.tsx`에 `viewMode: 'list' \| 'create' \| 'running' \| 'results'` 확장. |
| **IPC 네이밍** | collab-board 미러 / 독자 prefix | **`realtime-wall:*` prefix 고수** | 기존 5채널(`:start/:stop/:tunnel-*/:student-submitted/:connection-count`)과 연속성. collab-board `:list/:create/:rename/:delete/:start-session/:end-session/:get-active-session/:save-snapshot` 구조 그대로 네이밍 미러. |
| **결과(ToolResult)와 보드의 관계** | ToolResult에 `boardId` 필드 추가 / 별도 인덱스 테이블 | **ToolResult에 optional `boardId` 추가** | 최소 침습. `RealtimeWallResultData`에 `boardId?: string` 추가. 기존 결과는 optional이라 hydration 가능. |
| **State Management** | 신규 스토어 / 기존 스토어 확장 | **신규 3개 스토어** | `useRealtimeWallPresetStore`(M1), `useRealtimeWallBoardStore`(M3), `useRealtimeWallSessionStore`(M2 런타임 상태). 관심사 분리. |
...
├── domain/
│   ├── entities/
│   │   ├── RealtimeWallPreset.ts       [M1 신규]
│   │   ├── RealtimeWallSnapshot.ts     [M2 신규]
│   │   └── BulletinBoard.ts                [M3 신규]
│   ├── valueObjects/
│   │   └── BulletinBoardId.ts              [M3 신규]
│   ├── ports/
│   │   └── IRealtimeWallSnapshotPort.ts[M2 신규]
│   ├── repositories/
│   │   ├── IRealtimeWallPresetRepository.ts [M1]
│   │   └── IBulletinBoardRepository.ts     [M3 신규]
│   └── rules/
│       └── realtimeWallRules.ts        [기존 — 확장 함수 추가: buildPresetFromConfig 등]
│
├── usecases/
│   └── realtimeWall/
│       ├── ManagePresets.ts                [M1]
...
├── adapters/
│   ├── repositories/
│   │   ├── JsonRealtimeWallPresetRepository.ts [M1]
│   │   └── FileBulletinBoardRepository.ts  [M3]
│   ├── stores/
│   │   ├── useRealtimeWallPresetStore.ts [M1]
│   │   ├── useRealtimeWallBoardStore.ts  [M3]
│   │   └── useRealtimeWallSessionStore.ts[M2]
...
electron/ipc/
└── realtimeWall.ts                     [기존 — IPC 채널 대거 추가]

수정 파일(기존):
- src/adapters/components/Tools/ToolRealtimeWall.tsx  (뷰 모드 확장)
...
|----------|---------------|-----------|:--------:|
| **IPC 채널 네이밍** | 기존: `realtime-wall:start/stop/...` | `realtime-wall:list/create/rename/delete/start-session/end-session/get-active-session/save-snapshot/load-snapshot/list-snapshots/delete-snapshot` 추가 | High |
...
2. `RealtimeWallPreset` 엔티티 + 도메인 규칙 추가
3. `useToolPresetStore` 확장 (또는 신규 `useRealtimeWallPresetStore`)
4. `CreateView`에 PresetPicker 드롭다운 + "프리셋으로 저장" 버튼
5. PresetManagerModal (목록/이름변경/삭제)
6. 단위 테스트

### Phase M2 — 자동 저장/재개 (3~5일 예상)
1. `RealtimeWallSnapshot` 엔티티 + `IRealtimeWallSnapshotPort`
2. `FileBulletinSnapshotPersistence` (Electron 어댑터)
3. IPC 4채널 신설 + preload/global.d.ts 동기
4. `ToolRealtimeWall.tsx`에 30초 interval + 3초 debounce autosave 훅
...
4. `useRealtimeWallBoardStore`
5. `ToolRealtimeWall.tsx`에 `viewMode: 'list'` 추가 + `BulletinBoardList.tsx` 구현
...
- ❌ `tool-realtime-wall` 외에 새로운 Sidebar PageId 추가 (예: `tool-realtime-wall-manage`)
- ❌ collab-board 코드를 직접 import해서 공유 (두 도구는 엔티티/저장 경로/IPC prefix 모두 독립)
- ❌ realtime-wall 기존 5채널 IPC 이름 변경 (하위 호환 깨짐)
...
실시간 담벼락의 관리 기능은 **"프리셋(M1) → 자동 저장/재개(M2) → 보드 엔티티화(M3)"** 순으로 단계적으로 쌓으며, **collab-board의 14채널 IPC 패턴과 useToolPresetStore 확장 패턴을 미러링**해 최소 설계 비용으로 "여러 개 개설 + 저장 + 수정관리" 3축을 모두 확보한다. BETA 배지는 M3 완료 시점에 제거 검토한다.
...
2. [ ] `/pdca design realtime-wall-management` — M1·M2·M3의 상세 설계 문서 작성 (IPC 채널 signature, 엔티티 필드 확정, UI 컴포넌트 시그니처)
3. [ ] Phase M1 Do 진입 — 프리셋부터 구현·테스트·릴리즈
4. [ ] 각 Phase 완료마다 `/pdca analyze realtime-wall-management` Match Rate 측정 → 90%↑ 시 다음 Phase
