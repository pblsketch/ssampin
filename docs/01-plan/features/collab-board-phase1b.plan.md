...
Phase 1a는 ToolCollabBoard·FeedbackWallView에 베타 배너 + "개발자에게 피드백" mailto 링크를 박았다. Phase 1b는:
- 1a 배너를 **유지**할지 / 제거할지 판단 (베타 졸업 시점이 Phase 1b 완료 시점과 맞는지)
- mailto가 아닌 **구조화된 피드백 수집** (노션 폼 / Google Form) 도입 여부 검토
...
1. **2~4주간 베타 피드백 수집** — FeedbackWallView·ToolCollabBoard의 "개발자에게 피드백" 링크를 통해 들어오는 이메일 + 직접 요청한 교사 3~5명 인터뷰
2. **피드백 분류** — 버그(hotfix 대상) / UX 개선(1b 범위) / 기능 요청(1b or 2)
