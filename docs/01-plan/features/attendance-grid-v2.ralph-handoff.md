...
- [ ] PROGRESS.md 갱신 + DECISIONS.md ADR 추가(v2 결정 — 계획서 §7 초안 참조)
- [ ] v1 커밋 9개+v2 커밋 **전체 미푸시** — 사용자에게 푸시 여부 확인
- [ ] 릴리즈 노트 고지 필수 2건: ① 단일 날짜 카드 출결 → '오늘 출결' 그리드/출결 탭 이동 ② 기록 탭 첫 진입 화면이 '출결'로 변경
- [ ] /docs 사용자 가이드(landing/src/content/docs.ts) 출결 섹션 갱신 — 릴리즈 작업 단위에서
- [ ] 실기기(사용자) 확인 요청: 팔레트 입력·텍스트 입력·좌석 뷰·다크/라이트
- [ ] 모바일 행 모델 이관·모둠/자유 좌석 렌더 — 후속 과제로 PROGRESS에 기록
