...
4. **A1** — **끌기 자리(가운데 점 표시)에 커서를 올려도 패널이 열리지 않는가.** 거기서 끌면 8단계로 스냅되는가. 손을 뗀 뒤에도 열리지 않는가. 앱을 껐다 켜도 위치가 유지되는가. 배율이 다른 모니터에서도 화면 밖으로 나가지 않는가.
5. **A1 시인성** — 32 DIP 점 표시가 손잡이 폭 30~52 DIP 안에서 알아볼 만한가. 여는 버튼 두 개와 헷갈리지 않는가.
