...
> ⚠️ **§1~§9는 착수 시점의 계획이고, 실기기 QA를 거치며 A1·A2가 바뀌었다. 실제 동작은 §11을 본다.**
>
> - **A1**: 8칸 스냅을 버렸다. 놓은 자리를 그대로 쓴다(`railPosition` 0~1 비율). 이유는 §11.3.3.
> - **A1**: 여는 자리와 끌기 자리를 나눴다. 손잡이 가운데 32 DIP만 끌기용이다. 이유는 §11.3.
> - **A2**: 판정 폭이 손잡이 폭 전체다(가운데 44 DIP가 아니다). 이유는 §11.3.3 원인 3.
